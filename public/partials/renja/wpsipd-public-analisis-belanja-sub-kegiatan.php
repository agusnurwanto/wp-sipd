...
$nama_excel = 'Analisis Belanja per-SUB KEGIATAN '.$input['tahun_anggaran'].' '.strtoupper($nama_pemda);
...
$sql = "
    ...
    ".$where_jadwal." 
    ".$where_skpd."
    GROUP by kode_sub_giat
    ORDER BY kode_sub_giat ASC";
$analisis_sub_giat = $wpdb->get_results($wpdb->prepare($sql,$input['tahun_anggaran']), ARRAY_A);

$data_all = array(
    'total' => 0,
    'data'  => array()
);
if(!empty($analisis_sub_giat)){
    foreach($analisis_sub_giat as $k => $ap){
        if(empty($data_all['data'])){
            $data_all['data'] = $analisis_sub_giat;
        }
...
<table cellpadding="2" cellspacing="0" style="font-family:\'Open Sans\',-apple-system,BlinkMacSystemFont,\'Segoe UI\',sans-serif; border-collapse: collapse; width: 100%; table-layout: fixed; overflow-wrap: break-word; font-size: 60%; border: 0;">
    <thead>
        <tr>    
            ...
            <th class="atas kiri kanan bawah text_tengah">Sub Kegiatan</th>
