...
class Wpsipd_Public extends Wpsipd_Public_Base_1
{
				...
				if(in_array("administrator", $user_meta->roles)){
					if(!empty($_POST['nama']) && !empty($_POST['jadwal_mulai']) && !empty($_POST['jadwal_selesai']) && !empty($_POST['tahun_anggaran']) && !empty($_POST['tipe_perencanaan'])){
						...
						if(!empty($sqlTipe)){
							...
							$sqlSameTipe = $wpdb->get_results("SELECT * FROM `data_jadwal_lokal` WHERE id_tipe='".$id_tipe."' AND status = 0", ARRAY_A);
	...
	function update_visi_rpjm(){
		...
		if (!empty($_POST)) {
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option( '_crb_api_key_extension' )) {
				...
				$id_cek = $wpdb->get_var("
					SELECT id FROM data_rpjmd_visi_lokal
						WHERE visi_teks='".$data['visi_teks']."'
									AND id != ".$data['id_visi']."
									AND is_locked=0
									AND status=1
									AND active=1
	...
	function get_misi_rpjm(array $input){
		global $wpdb;
				...
				if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option( '_crb_api_key_extension' )) {
					...
					$id_cek = $wpdb->get_var("
						SELECT id FROM data_rpjmd_misi_lokal
							WHERE misi_teks LIKE '%".$data['misi_teks']."%' 
										AND id_visi=".$data['id_visi']."
										AND is_locked=0
										AND status=1
										AND active=1
								");
					...
					$dataVisi = $wpdb->get_row("SELECT id AS id_visi, visi_teks, is_locked AS visi_lock FROM data_rpjmd_visi_lokal WHERE id=".$data['id_visi'] . " AND is_locked=0 AND status=1 AND active=1");
					...
					$dataMisi = $this->get_misi_rpjm($data);
		...
		try{
			if (!empty($_POST)) {
				if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option( '_crb_api_key_extension' )) {
					...
					$id_cek = $wpdb->get_var("
						SELECT id FROM data_rpjmd_misi_lokal
							WHERE misi_teks='".$data['misi_teks']."' 
									AND id!=".$data['id_misi']."
									AND is_locked=0
									AND status=1
									AND active=1");
	...
	function get_tujuan_rpjm(array $input){
		global $wpdb;
				...
				if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option( '_crb_api_key_extension' )) {
					...
					$id_cek = $wpdb->get_var("
						SELECT id FROM data_rpjmd_tujuan_lokal
							WHERE tujuan_teks LIKE '%".$data['tujuan_teks']."%'
					...
					$dataMisi = $wpdb->get_row("SELECT id AS id_misi, misi_teks, is_locked AS misi_lock, id_visi, urut_misi FROM data_rpjmd_misi_lokal WHERE id=".$data['id_misi'] . " AND is_locked=0 AND status=1 AND active=1");
					...
					$dataTujuan = $this->get_tujuan_rpjm($data);

					echo json_encode([
						'status' => true,
						'message' => 'Sukses simpan tujuan',
						'data' => $dataTujuan['html'],
						'sql' => $dataTujuan['sql'],
					]);exit;
		...
		try{
			if (!empty($_POST)) {
				if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option( '_crb_api_key_extension' )) {
					...
					$id_cek = $wpdb->get_var("
						SELECT id FROM data_rpjmd_tujuan_lokal
							WHERE tujuan_teks='".$data['tujuan_teks']."'
										AND id!=".$data['id_tujuan']."
										AND is_locked=0
										AND status=1
										AND active=1
								");
					...
					if(!$status){
						throw new Exception('Terjadi kesalahan update data, hubungi admin');
					}
	...
	function kelola_indikator_tujuan_rpjm(){
		global $wpdb;
		...
		try{
			if (!empty($_POST)) {
				if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option( '_crb_api_key_extension' )) {
					...
					if(empty($indikators)){
						// ambil data tujuan
						$indikators = $wpdb->get_results("select * from data_rpjmd_tujuan_lokal where id=".$_POST['id_tujuan']. " and id_unik is not null and id_unik_indikator is null and is_locked_indikator=0 and status=1 and active=1", ARRAY_A);
					}
		...
		try{
			if (!empty($_POST)) {
				if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option( '_crb_api_key_extension' )) {
					...
					$dataTujuan = $wpdb->get_row("SELECT * FROM data_rpjmd_tujuan_lokal WHERE id=".$data['id_tujuan'] . " AND is_locked=0 AND status=1 AND active=1 AND id_unik IS NOT NULL AND id_unik_indikator IS NULL");
					...
					$dataIndikatorTujuan = $this->get_indikator_tujuan_rpjm($data);
		...
		try{
			if (!empty($_POST)) {
				if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option( '_crb_api_key_extension' )) {
					...
					$data = $this->get_indikator_tujuan_rpjm([
						'type' => 1,
	...
	function get_indikator_tujuan_rpjm(array $input){
		
		global $wpdb;

		$data = $wpdb->get_results("select * from data_rpjmd_tujuan_lokal where id_tujuan=".$input['id_tujuan']." and id_unik_indikator is not null  and active=1 and is_locked_indikator=0 and status=1 order by id", ARRAY_A);
	...
	function get_sasaran_rpjm(array $input){
		global $wpdb;
		...
		if($type == 1){
					$sql = $wpdb->prepare("
						...
						from data_rpjmd_sasaran_lokal
						where is_locked=0
							AND kode_tujuan='".$input['kode_tujuan']."'
							AND id_unik IS NOT NULL
							AND id_unik_indikator IS NULL
							AND is_locked=0
							AND status=1
		...
		try{
			if (!empty($_POST)) {
				if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option( '_crb_api_key_extension' )) {
					...
					$dataTujuan = $wpdb->get_row("SELECT id_unik, tujuan_teks, is_locked AS tujuan_lock, id_misi, urut_tujuan FROM data_rpjmd_tujuan_lokal WHERE id_unik='".$data['kode_tujuan'] . "' AND is_locked=0 AND status=1 AND active=1 AND id_unik IS NOT NULL AND id_unik_indikator IS NULL");
					...
					$dataSasaran = $this->get_sasaran_rpjm($data);

					echo json_encode([
						'status' => true,
						'data' => $dataSasaran['html'],
	...
	function kelola_indikator_sasaran_rpjm(){
		global $wpdb;
		...
		try{
			if (!empty($_POST)) {
				if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option( '_crb_api_key_extension' )) {
					...
					// ambil data indikator
					$indikators = $wpdb->get_results("select * from data_rpjmd_sasaran_lokal where id_sasaran=".$_POST['id_sasaran']. " and id_unik is not null and id_unik_indikator is not null and is_locked_indikator=0 and status=1 and active=1", ARRAY_A);

					if(empty($indikators)){
						// ambil data sasaran
						$indikators = $wpdb->get_results("select * from data_rpjmd_sasaran_lokal where id=".$_POST['id_sasaran']. " and id_unik is not null and id_unik_indikator is null and is_locked_indikator=0 and status=1 and active=1", ARRAY_A);
					}
					...
					foreach ($indikators as $key => $value) {
						...
						if(empty($data[$value['id_unik']])){
							$data[$value['id_unik']] = [
								'id' => $value['id'],
								'id_unik' => $value['id_unik'],
								'sasaran_teks' => $value['sasaran_teks'],
								'indikator'=> []
							];
						}
	...
	function get_indikator_sasaran_rpjm(array $input){
		global $wpdb;

		$data = $wpdb->get_results("select * from data_rpjmd_sasaran_lokal where id_sasaran=".$input['id_sasaran']." and id_unik_indikator is not null and active=1 and is_locked_indikator=0 and status=1 order by id", ARRAY_A);
			...
			if (!empty($_POST)) {
				if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option( '_crb_api_key_extension' )) {
					...
					$id_cek = $wpdb->get_var("
						SELECT id FROM data_rpjmd_sasaran_lokal
							WHERE indikator_teks='".$data['indikator_teks']."'
										AND id_sasaran=".$data['id_sasaran']."
										AND is_locked_indikator=0
										AND status=1
										AND active=1
								");
					...
					$dataSasaran = $wpdb->get_row("SELECT * FROM data_rpjmd_sasaran_lokal WHERE id=".$data['id_sasaran'] . " AND is_locked=0 AND status=1 AND active=1 AND id_unik IS NOT NULL AND id_unik_indikator IS NULL");

					if (empty($dataSasaran)) {
						throw new Exception('Sasaran yang dipilih tidak ditemukan!');
					}

					$dataTujuan = $wpdb->get_row("SELECT * FROM data_rpjmd_tujuan_lokal WHERE id_unik='".$dataSasaran->kode_tujuan . "' AND is_locked=0 AND status=1 AND active=1 AND id_unik IS NOT NULL AND id_unik_indikator IS NULL");
					...
					$dataIndikatorSasaran = $this->get_indikator_sasaran_rpjm($data);

					echo json_encode([
						'status' => true,
						'message' => 'Sukses simpan indikator sasaran',
						'data' => $dataIndikatorSasaran['html'],
					]);exit;
		...
		try{
			if (!empty($_POST)) {
				if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option( '_crb_api_key_extension' )) {
					...
					$indikator = $wpdb->get_row("select * from data_rpjmd_sasaran_lokal where id=".$_POST['id']." and id_sasaran=".$_POST['id_sasaran']. " and id_unik is not null and id_unik_indikator is not null and is_locked_indikator=0 and status=1 and active=1");
	...
	function add_program_rpjm(){
		...
		$ret = array(
			'status'	=> 'success',
			'message'	=> 'Add program RPJM!'
		);
		if (!empty($_POST)) {
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option( '_crb_api_key_extension' )) {
				
				$tahun_anggaran = $_POST['tahun_anggaran'] ?? date('Y');
				$type = $_POST['type'];
				...
				if($type == 1){
					$sql = $wpdb->prepare("
						...
						where is_locked=0
							...
							AND active=1
					");
					$ret['sasaran'] = $wpdb->get_results($sql, ARRAY_A);
				}else{
					$sql = $wpdb->prepare("
						select 
							* 
				...
				$ret['units'] = $wpdb->get_results("SELECT * FROM data_unit WHERE tahun_anggaran=".$tahun_anggaran." and active=1 and is_skpd=1 order by id_skpd ASC;", ARRAY_A);
			}else{
				$ret = array(
					'status' => 'error',
					'message'	=> 'Api Key tidak sesuai!'
	...
	function edit_program_rpjm(){
		...
		if (!empty($_POST)) {
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option( '_crb_api_key_extension' )) {
				...
				$units = $wpdb->get_results("SELECT * FROM data_unit WHERE tahun_anggaran=".date('Y')." and active=1 and is_skpd=1 order by id_skpd ASC;", ARRAY_A);
				...
				echo json_encode([
					...
					'sasaran' => $sasaran,
					'program' => $program,
					'message' => 'Sukses get program by id'
				]);exit;
			}
		...
		try{
			if (!empty($_POST)) {
				if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option( '_crb_api_key_extension' )) {
					...
					$dataUnit = $wpdb->get_row("SELECT * FROM data_unit WHERE id_unit=".$data['id_unit']." and tahun_anggaran=".date('Y')." and active=1 and is_skpd=1 order by id_skpd ASC;");
					...
					$id_cek = $wpdb->get_var("
						SELECT id FROM data_rpjmd_program_lokal
							WHERE nama_program LIKE '".$data['program_teks']."%'
										AND indikator LIKE '%".$data['indikator_teks']."' 
					...
					if(!empty($id_cek)){
						throw new Exception('Program & indikator : '.$data['program_teks'].' & '.$data['indikator_teks'].' sudah ada!');
					}

					$dataSasaran = $wpdb->get_row("SELECT id_unik, sasaran_teks, is_locked AS sasaran_lock, urut_sasaran, kode_tujuan FROM data_rpjmd_sasaran_lokal WHERE id_unik='".$data['kode_sasaran'] . "' AND is_locked=0 AND status=1 AND active=1");
					...
					if(empty($dataTujuan)){
						throw new Exception('Tujuan dari sasaran yang dipilih tidak ditemukan!');
					}

					$dataMisi = $wpdb->get_row("SELECT id AS id_misi, misi_teks, is_locked AS misi_lock, id_visi, urut_misi FROM data_rpjmd_misi_lokal WHERE id=".$dataTujuan->id_misi . " AND is_locked=0 AND status=1 AND active=1");

					if(empty($dataMisi)){
						throw new Exception('Misi dari sasaran yang dipilih tidak ditemukan!');
					}

					$dataVisi = $wpdb->get_row("SELECT id AS id_visi, visi_teks, is_locked AS visi_lock FROM data_rpjmd_visi_lokal WHERE id=".$dataMisi->id_visi . " AND is_locked=0 AND status=1 AND active=1");

					if(empty($dataVisi)){
						throw new Exception('Visi dari sasaran yang dipilih tidak ditemukan!');
					}

					$status = $wpdb->insert('data_rpjmd_program_lokal', [
						'id_misi' => $dataMisi->id_misi,
					...
					if(!$status){
						throw new Exception('Terjadi kesalahan saat simpan program, harap hubungi admin!');
					}

					echo json_encode([
						'status' => true,
						'message' => 'Sukses simpan program dan indikator'
					]);exit;
		...
		try{
			if (!empty($_POST)) {
				if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option( '_crb_api_key_extension' )) {
					...
					$id_cek = $wpdb->get_var("
						SELECT id FROM data_rpjmd_program_lokal
							WHERE nama_program = '".$data['program_teks']."'
										AND indikator = '".$data['indikator_teks']."' 
										AND kode_sasaran!='".$data['kode_sasaran']."'
										AND program_lock=0
										AND status=1
										AND active=1
					...
					$status = $wpdb->update('data_rpjmd_program_lokal', [
						'id_misi' => $dataMisi->id_misi,
						'id_unit' => $dataUnit->id_unit,
						'id_visi' => $dataMisi->id_visi,
						'indikator' => $data['indikator_teks'],
					...
					echo json_encode([
						'status' => true,
						'message' => 'Sukses ubah program dan indikator'
					]);exit;
	...
	function verify_program_rpjm(array $data){
		...
		if(empty($data['program_teks'])){
			throw new Exception('Program tidak boleh kosong!');
		}

		if(empty($data['urut_program'])){
			throw new Exception('Urut program tidak boleh kosong!');
		}
		...
		if(empty($data['pagu_2'])){
			throw new Exception('Pagu program tahun ke-2 tidak boleh kosong!');
		}

		if(empty($data['pagu_3'])){
			throw new Exception('Pagu program tahun ke-3 tidak boleh kosong!');
		}

		if(empty($data['pagu_4'])){
			throw new Exception('Pagu program tahun ke-4 tidak boleh kosong!');
		}
