...
class Wpsipd_Public_Base_2 extends Wpsipd_Public_Base_3
{
			...
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
				if (!empty($_POST['tahun_anggaran']) && !empty($_POST['id_lokasi'])) {
					...
					foreach ($skpd as $k => $opd) {
						...
						$total_paket_penyedia_sirup = $wpdb->get_results(
							$wpdb->prepare(
								'
							SELECT
						...
						$total_pagu_rekap_penyedia_sirup = $wpdb->get_results(
							$wpdb->prepare(
								'
							SELECT
								pagu_penyedia
						...
						$data_total_pagu_rekap_sirup = $wpdb->get_results(
							$wpdb->prepare(
								'
							SELECT 
								total_pagu
	...
	public function get_data_program_renstra()
	{
		...
		if (!empty($_POST)) {
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
				...
				$data_sasaran_rpd_history = $wpdb->get_results(
					$wpdb->prepare(
				...
				$data_renstra_program = $wpdb->get_results(
					$wpdb->prepare(
						'SELECT
						p.*,
						t.kode_sasaran_rpjm
				...
				foreach ($data_renstra_program as $v_renstra) {
					...
					if (empty($v_renstra['id_unik_indikator'])) {
						$data_rpd_same_program = $wpdb->get_results(
							$wpdb->prepare(
								'SELECT *
							FROM ' . $program_parent . ' 
							WHERE 
						...
						//simpan indikator
						$data_rpd_same_indikator = $wpdb->get_results(
							$wpdb->prepare(
								'SELECT *
							FROM ' . $program_parent . ' 
							WHERE 
				...
				if ($ret['status'] != 'error') {
					...
					$data_akun = $wpdb->get_results($wpdb->prepare(
						...
						WHERE tahun_anggaran=%d
							...
							AND set_input=1
					',
						$data_pendapatan_by_id['tahun_anggaran']
					), ARRAY_A);
				...
				if ($ret['status'] != 'error') {
					...
					$data_akun = $wpdb->get_results($wpdb->prepare(
						...
						WHERE tahun_anggaran=%d
							...
							AND set_input=1
					',
						$data_penerimaan_by_id['tahun_anggaran']
					), ARRAY_A);
				...
				if ($ret['status'] != 'error') {
					...
					$data_akun = $wpdb->get_results($wpdb->prepare(
						...
						WHERE tahun_anggaran=%d
							...
							AND set_input=1
					',
						$data_pengeluaran_by_id['tahun_anggaran']
					), ARRAY_A);
	...
	function get_rinc_rka_lokal_pembiayaan()
	{
		global $wpdb;
		$return = array(
			'status' => 'success',
		...
		if (!empty($_POST)) {
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
				...
				if (!empty($_POST['tipe']) && $_POST['tipe'] == 'pembiayaan') {
					$type_apbd = $_POST['tipe'];
					$rinc = $wpdb->get_results($wpdb->prepare("
						SELECT 
							*,
							...
							nilaimurni as total_harga_murni,
							'Tahun' as satuan
						from data_pembiayaan" . $prefix_tabel . " 
						where id_skpd=%d
							AND tahun_anggaran=%d
							AND active=1
						Order by kode_akun ASC, subs_bl_teks ASC, ket_bl_teks ASC
			...
			if (!empty($_POST)) {
				if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
					...
					if ($jenis == 'tujuan') {
	                    ...
	                    $data_transformasi_cascading = $wpdb->get_results(
	                        $wpdb->prepare("
	                            ...
	                            WHERE id_jadwal = %d
	                              AND id_skpd = %d
	                              AND active = 1
	                        ", $data_jadwal_renstra['id_jadwal_lokal'], $_POST['id_skpd']),
	                        ARRAY_A
	                    );
	                    foreach ($data_transformasi_cascading as &$cascading) {
	                        $cascading['program'] = $wpdb->get_results(
	                            $wpdb->prepare("
	                                SELECT 
	                                	*
	                    ...
	                    if (!empty($data_transformasi_cascading)) {
	                        $ret['data_transformasi_cascading'] = $data_transformasi_cascading;
	                    } else {
	                        $ret['data_transformasi_cascading'] = array();
	                    }
	                    ...
	                    foreach ($data_transformasi_cascading as &$cascading) {
	                        $cascading['kegiatan'] = $wpdb->get_results(
	                            $wpdb->prepare("
	                                SELECT 
	                                	*
	                                FROM data_progkeg_transformasi_cascading
	                    ...
	                    foreach ($data_transformasi_cascading as &$cascading) {
	                        $cascading['sub_kegiatan'] = $wpdb->get_results(
	                            $wpdb->prepare("
	                                SELECT 
	                                	*
	                                FROM data_progkeg_transformasi_cascading
