...
$akun_hibah_uang = $wpdb->get_results('SELECT id_akun, kode_akun, nama_akun FROM `data_akun` where is_bankeu_umum=1 and 1=0 and tahun_anggaran='.$input['tahun_anggaran'].' order by kode_akun ASC', ARRAY_A);
...
$akun_hibah_brg = $wpdb->get_results('SELECT id_akun, kode_akun, nama_akun FROM `data_akun` where is_bankeu_khusus=1 and 1=0 and tahun_anggaran='.$input['tahun_anggaran'].' order by kode_akun ASC', ARRAY_A);
...
?>
<div id="cetak" title="Laporan APBD PENJABARAN Lampiran 6a Tahun Anggaran <?php echo $input['tahun_anggaran']; ?>" style="padding: 5px;">
    <table align="right" class="no-border no-padding" cellspacing="0" cellpadding="0" style="width:280px; font-size: 12px;">
        <tr>
            <td width="80" valign="top">Lampiran VIa </td>
    ...
    <h4 style="text-align: left; font-size: 13px; font-weight: bold;">
    <h4 style="text-align: center; font-size: 13px; margin: 10px auto; min-width: 450px; max-width: 570px; font-weight: bold;">DAFTAR NAMA CALON PENERIMA, ALAMAT DAN BESARAN<br>PERUBAHAN ALOKASI BELANJA BAGI HASIL PAJAK DAERAH KEPADA PEMERINTAH KABUPATEN <br> <?php echo $input['tahun_anggaran']; ?></h4>
