...
class Wpsipd_Public_Base_3 extends Wpsipd_Public_Ssh
{
	...
	public function detail_tujuan_sasaran_manrisk_pemda($atts)
	{
		...
		require_once WPSIPD_PLUGIN_PATH . 'public/partials/renstra/wpsipd-public-detail-tujuan-sasaran-manrisk-pemda.php';
	}
	...
	public function monitor_resiko($atts)
	{
		...
		require_once WPSIPD_PLUGIN_PATH . 'public/partials/renstra/wpsipd-public-monitor-resiko-manrisk.php';
	}
	...
	public function get_table_tujuan_sasaran()
	{
	    ...
	    if (!empty($_POST)) {
	        if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
	            ...
	            // Get data manrisk OPD
	            $get_data_opd = $wpdb->get_results($wpdb->prepare("
	                SELECT 
	                    * 
	            ...
	            $id_tujuan_pemda_terhubung = array();
	            $id_sasaran_pemda_terhubung = array();
	            $kode_bidang_opd = array();

	            foreach ($get_data_opd as $row_opd) {
	                if (isset($row_opd['tipe']) && $row_opd['tipe'] == 0) {
	                    // Dari Tujuan OPD
	                    $get_data_tujuan = $wpdb->get_row($wpdb->prepare("
	                        SELECT 
	                            kode_bidang_urusan_multiple,
	                            kode_sasaran_multiple
	                        FROM data_renstra_tujuan
	                        WHERE id_unik = %d
	                            AND id_jadwal = %d
	                            AND id_unit = %d
	                            AND active = 1
	                    ", $row_opd['id_tujuan_sasaran'], $id_jadwal, $id_skpd), ARRAY_A);
	                    
	                    if (!empty($get_data_tujuan)) {
	                        // Simpan kode bidang urusan
	                        $kode_bidang_multiple = json_decode($get_data_tujuan['kode_bidang_urusan_multiple'], true);
	                        if (!empty($kode_bidang_multiple) && is_array($kode_bidang_multiple)) {
	                            $kode_bidang_opd = array_merge($kode_bidang_opd, $kode_bidang_multiple);
	                        }
	                ...
	                } elseif (isset($row_opd['tipe']) && $row_opd['tipe'] == 1) {
	                    // Dari Sasaran OPD
	                    ...
	                    if (!empty($get_data_sasaran)) {
	                        $get_tujuan_parent = $wpdb->get_row($wpdb->prepare("
	                            SELECT 
	                                kode_bidang_urusan_multiple,
	                                kode_sasaran_multiple
	                            FROM data_renstra_tujuan
	                            WHERE id_unik = %s
	                                ...
	                                AND id_unit = %d
	                                AND active = 1
	                        ", $get_data_sasaran['kode_tujuan'], $id_jadwal, $id_skpd), ARRAY_A);
	                        
	                        if (!empty($get_tujuan_parent)) {
	                            // Simpan kode bidang urusan
	                            $kode_bidang_multiple = json_decode($get_tujuan_parent['kode_bidang_urusan_multiple'], true);
	                            if (!empty($kode_bidang_multiple) && is_array($kode_bidang_multiple)) {
	                                $kode_bidang_opd = array_merge($kode_bidang_opd, $kode_bidang_multiple);
	                            }
	            ...
	            $kode_bidang_opd = array_unique($kode_bidang_opd);
	            $id_tujuan_pemda_terhubung = array_unique($id_tujuan_pemda_terhubung);
	            $id_sasaran_pemda_terhubung = array_unique($id_sasaran_pemda_terhubung);

	            $html = '';
	            $data_sesudah = false;
	            
	            // AMBIL DATA PEMDA YANG TERHUBUNG
	            ...
	            if (!empty($id_jadwal_relasi) && !empty($jenis_jadwal_relasi)) {
	                ...
	                if (!empty($get_data_pemda)) {
	                    foreach ($get_data_pemda as $row_pemda) {
	                        ...
	                        // Skip jika data tidak terhubung
	                        if ($skip_data) {
	                            continue;
	                        }
	                        ...
	                        foreach ($kode_bidang_opd as $kode) {
	                            if (!isset($data_pemda_per_bidang[$kode])) {
	                                $data_pemda_per_bidang[$kode] = array();
	                            }
	                            
	                            $tujuan_sasaran_key = $row_pemda['id_tujuan_sasaran'] . '_' . $row_pemda['tipe'];
	                            
	                            if (!isset($data_pemda_per_bidang[$kode][$tujuan_sasaran_key])) {
	                                $data_pemda_per_bidang[$kode][$tujuan_sasaran_key] = array(
	                            ...
	                            $indikator_key = $row_pemda['id_indikator'];
	                            if (!isset($data_pemda_per_bidang[$kode][$tujuan_sasaran_key]['indikator'][$indikator_key])) {
	                                $data_pemda_per_bidang[$kode][$tujuan_sasaran_key]['indikator'][$indikator_key] = array(
	                                    'indikator_text' => $row_pemda['indikator'],
	                                    'data' => array()
	                                );
	                            }
	                            
	                            $data_pemda_per_bidang[$kode][$tujuan_sasaran_key]['indikator'][$indikator_key]['data'][] = $row_pemda;
	                        }
	                    }
	                }
	            }

	            // PROSES KONEKSI OPD DAN GABUNGKAN DENGAN PEMDA
	            if (!empty($get_data_opd)) {
	                $grouped_data = array();
	                $tujuan_sasaran_groups_opd = array();
	                
	                foreach ($get_data_opd as $row) {
	                    $kode_bidang_array = array();
	                    $nama_bidang = '';
	                    $nama_tujuan_sasaran = '';
	                    $kode_bidang = '';
	                    ...
	                    if (isset($row['tipe']) && $row['tipe'] == 0) {
	                        $get_data_tujuan = $wpdb->get_row($wpdb->prepare("
	                            ...
	                            WHERE id_unik = %d
	                                ...
	                                AND id_unit = %d
	                                AND active = 1
	                        ", $row['id_tujuan_sasaran'], $id_jadwal, $id_skpd), ARRAY_A);
	                        ...
	                        if (!empty($get_data_tujuan)) {
	                            ...
	                            $kode_bidang_multiple = json_decode($get_data_tujuan['kode_bidang_urusan_multiple'], true);
	                            if (!empty($kode_bidang_multiple) && is_array($kode_bidang_multiple)) {
	                                $kode_bidang_array = $kode_bidang_multiple;
	                            }
	                    ...
	                    if (!empty($kode_bidang_array)) {
	                        $nama_bidang_array = array();
	                        
	                        foreach ($kode_bidang_array as $kode) {
	                        ...
	                        $nama_bidang = !empty($nama_bidang_array) ? implode(', ', $nama_bidang_array) : '';
	                        $kode_bidang = implode(',', $kode_bidang_array);
	                        
	                        if (!empty($kode_bidang)) {
	                            $grouped_data[$kode_bidang]['nama_bidang'] = $nama_bidang;
	                            $grouped_data[$kode_bidang]['kode_bidang_array'] = $kode_bidang_array;
	                ...
	                uksort($grouped_data, function($a, $b) {
	                    ...
	                    $a_clean = str_replace(',', '.', (string)$a_first);
	                    $b_clean = str_replace(',', '.', (string)$b_first);
	                ...
	                foreach ($grouped_data as $kode_group => $group) {
	                    $html .= '
	                        <tr style="background:#f0f0f0; font-weight:bold;">
	                            <td colspan="31">' . $group['nama_bidang'] . '</td>
	                        </tr>
	                    ';

	                    $no = 1;
	                    
	                    $all_tujuan_sasaran = array();
	                    
	                    $kode_bidang_group = explode(',', $kode_group)[0];
	                    if (isset($data_pemda_per_bidang[$kode_bidang_group])) {
	                        foreach ($data_pemda_per_bidang[$kode_bidang_group] as $key => $pemda_group) {
	                            $all_tujuan_sasaran[$key] = $pemda_group;
	                            $all_tujuan_sasaran[$key]['jenis'] = 'pemda';
	                        }
	                    }
	                    
	                    if (isset($tujuan_sasaran_groups_opd[$kode_group])) {
	                        foreach ($tujuan_sasaran_groups_opd[$kode_group] as $key => $opd_group) {
	                            $all_tujuan_sasaran[$key] = $opd_group;
	                            $all_tujuan_sasaran[$key]['jenis'] = 'opd';
	                        }
	                    }
	                    
	                    // Urutan: Tujuan Pemda (0) -> Sasaran Pemda (1) -> Tujuan OPD (0) -> Sasaran OPD (1)
	                    uasort($all_tujuan_sasaran, function($a, $b) {
	                        $order_a = ($a['jenis'] == 'pemda' ? 0 : 2) + $a['tipe'];
	                        $order_b = ($b['jenis'] == 'pemda' ? 0 : 2) + $b['tipe'];
	                        return $order_a <=> $order_b;
	                    });
	                      
	                    foreach ($all_tujuan_sasaran as $tujuan_sasaran_group) {
	                        ...
	                        foreach ($tujuan_sasaran_group['indikator'] as $indikator_id => $indikator_data) {
	                            ...
	                            foreach ($indikator_data['data'] as $index => $data_sebelum) {
	                                ...
	                                $pemilik_resiko = isset($data_pemilik_resiko[$data_sebelum['pemilik_resiko']]) ? $data_pemilik_resiko[$data_sebelum['pemilik_resiko']] : '';
	                                $sumber_sebab = isset($data_sumber_sebab[$data_sebelum['sumber_sebab']]) ? $data_sumber_sebab[$data_sebelum['sumber_sebab']] : '';
	                                $pihak_terkena = isset($data_pihak_terdampak[$data_sebelum['pihak_terkena']]) ? $data_pihak_terdampak[$data_sebelum['pihak_terkena']] : '';
	                                ...
	                                // Cek apakah data dari pemda atau opd
	                                $is_pemda = ($tujuan_sasaran_group['jenis'] == 'pemda');
	                                ...
	                                if ($is_admin) {
		                                ...
		                                if (!empty($get_data_sesudah)) {
		                                    foreach ($get_data_sesudah as $data_sesudah_item) {
		                                        $id_sesudah = $data_sesudah_item['id'];
		                                        $tipe_sesudah = intval($data_sesudah_item['tipe']);
		                                        $id_tujuan_sesudah = $data_sesudah_item['id_tujuan_sasaran'];
		                                        $id_indikator_sesudah = $data_sesudah_item['id_indikator'];
		                                        ...
		                                        if ($data_sesudah_item['controllable'] == 0) {
		                                            $controllable_sesudah = 'Controllable';
		                                        } elseif ($data_sesudah_item['controllable'] == 1) {
		                                            $controllable_sesudah = 'Uncontrollable';
		                                        }

		                                        $pemilik_resiko_sesudah = isset($data_pemilik_resiko[$data_sesudah_item['pemilik_resiko']]) ? $data_pemilik_resiko[$data_sesudah_item['pemilik_resiko']] : '';
		                                        $sumber_sebab_sesudah = isset($data_sumber_sebab[$data_sesudah_item['sumber_sebab']]) ? $data_sumber_sebab[$data_sesudah_item['sumber_sebab']] : '';
		                                        $pihak_terkena_sesudah = isset($data_pihak_terdampak[$data_sesudah_item['pihak_terkena']]) ? $data_pihak_terdampak[$data_sesudah_item['pihak_terkena']] : '';
		          
		                                        $nilai_resiko_sesudah = $data_sesudah_item['skala_dampak']*$data_sesudah_item['skala_kemungkinan'];
		                                          
		                                        $html .= '
		                                            <td class="text-left">' . $data_sesudah_item['uraian_resiko'] . '</td>
		                                            <td class="text-left">' . $data_sesudah_item['kode_resiko'] . '</td>
		                                            <td class="text-left">' . $pemilik_resiko_sesudah . '</td>
		                                            <td class="text-left">' . $data_sesudah_item['uraian_sebab'] . '</td>
		                                            <td class="text-left">' . $sumber_sebab_sesudah . '</td>
		                                            <td class="text-left">' . $controllable_sesudah . '</td>
		                                            <td class="text-left">' . $data_sesudah_item['uraian_dampak'] . '</td>
		                                            <td class="text-left">' . $pihak_terkena_sesudah . '</td>
		                                            <td class="text-left">' . $data_sesudah_item['skala_dampak'] . '</td>
		                                            <td class="text-left">' . $data_sesudah_item['skala_kemungkinan'] . '</td>
		                                            <td class="text-left">' . $nilai_resiko_sesudah . '</td>
		                                            <td class="text-left">' . $data_sesudah_item['rencana_tindak_pengendalian'] . '</td>
		                                        ';
	                                ...
	                                if ($is_pemda) {
	                                    ...
	                                    $data_verifikasi = $wpdb->get_row($wpdb->prepare('
							                SELECT *
							                FROM data_keterangan_verifikator
							                WHERE id_manrisk = %d
							                    AND active = 1
							            ', $data_sebelum['id']), ARRAY_A);
							            ...
							            if (!empty($data_verifikasi)) {
										        ...
										        if (is_array($data_keterangan)) {
										            ...
										            if (!empty($keterangan_lama) && is_array($keterangan_lama)) {
										                $keterangan_lama = array_map(function ($ket_lama) {
										                    return is_array($ket_lama) ? $ket_lama : [
										                    	'tanggal' => '', 
											                    'pesan' => $ket_lama
										                    ];
										                }, $keterangan_lama);
										            ...
										            $wpdb->update(
										                'data_keterangan_verifikator',
										                ['keterangan_verifikasi' => $update_keterangan],
										                [
										                	'id_manrisk' => $data_sebelum['id'], 
										                	'active' => 1, 
										                	'tipe' => $tipe
										                ]
										            );

										            $keterangan_verifikasi = htmlspecialchars($keterangan['tanggal']) . '&nbsp;&nbsp;' . htmlspecialchars($keterangan['pesan']);
										    ...
										    } elseif ($data_verifikasi['status_verifikasi'] == 2) {
										        $color_badge_verify = 'warning';
										        $text_badge = 'Menunggu';
										        $show_button = true;

										    } elseif ($data_verifikasi['status_verifikasi'] == 3) {
										        $color_badge_verify = 'success';
										        $text_badge = 'DiSetujui';
										        $show_button = true;

										    } elseif ($data_verifikasi['status_verifikasi'] == 0) {
							            ...
							            $html .= '
										    <td>
										        <div class="text-center">
										            <span class="badge badge-' . $color_badge_verify . '" style="font-size:12px; padding:2px 5px; border-radius:4px;">
										                ' . $text_badge . '
										            </span>
										        </div>
										        ' . (!empty($keterangan_verifikasi) ? '
										            <div class="text-left">
										                ' . $keterangan_verifikasi . '
										            </div>
										        ' : '') . '
	                                    ...
	                                    $tahun_anggaran_jadwal = $jadwal_lokal['tahun_anggaran'];
	                                    ...
	                                    if (!empty($jadwal_lokal)) {
	                                        ...
	                                        if ($jenisJadwal == 'penetapan') {
	                                            ...
	                                            if ($now >= $awal && $now <= $akhir) {
	                                            	...
	                                            	if($is_admin) {	                                            		
		                                                if ($data_verifikasi['status_verifikasi'] == 2) {
		                                                    $html .= '
		                                                        <button class="btn btn-success" onclick="verifikasi_tujuan_sasaran_manrisk(\'' . $data_sebelum['id'] . '\', ' . $tipe_target . ',); return false;" title="Verifikasi Manrisk"><span class="dashicons dashicons-yes"></span>
															    </button>
															';
														} elseif ($data_verifikasi['status_verifikasi'] == 3){
		                                                    $html .= '
															    <button class="btn btn-primary" onclick="edit_tujuan_sasaran_manrisk_penetapan(' . $id_sesudah . ', ' . $data_sebelum['id'] . ', \'' . $id_tujuan . '\', \'' . $id_indikator . '\', ' . $tipe_sesudah . ', \'' . $get_kode_bidang . '\'); return false;" title="Edit Data"><span class="dashicons dashicons-edit"></span>
	                                                        	</button>
	                                                        ';

														}
	                                            	} else {
	                                            		if ($data_verifikasi['status_verifikasi'] != 3 && $data_verifikasi['status_verifikasi'] != 2){
			                                                $html .= '
			                                                        <button class="btn btn-primary" onclick="edit_tujuan_sasaran_manrisk(' . $data_sebelum['id'] . ', \'' . $id_tujuan . '\', \'' . $id_indikator . '\', ' . $tipe_target . ', \'' . $get_kode_bidang . '\'); return false;" title="Edit Data Sebelum">
			                                                            <span class="dashicons dashicons-edit"></span>
			                                                        </button>';
			                                                if ($draft) {
			                                                    $html .= '
			                                                        <button class="btn btn-info" onclick="usulkan_manrisk(\'' . $data_sebelum['id'] . '\', ' . $tipe_target . ',); return false;" title="Usulkan Manrisk"><span class="dashicons dashicons-upload"></span>
																    </button>
																';
															}	                                            			
	                                            		}
	                                            ...
	                                            if ($now >= $awal && $now <= $akhir) {
	                                            	...
	                                            	if($is_admin) {	                                            		
	                                            		...
	                                            		if ($data_verifikasi['status_verifikasi'] != 3 && $data_verifikasi['status_verifikasi'] != 2){
			                                                $html .= '
			                                                        <button class="btn btn-success" onclick="tambah_tujuan_sasaran_manrisk( \'' . $id_tujuan . '\', \'' . $id_indikator . '\', \'' . $tujuan_sasaran_group['nama_tujuan_sasaran'] . '\', \'' . $indikator_data['indikator_text'] . '\', ' . $tipe_target . ', \'' . $get_kode_bidang . '\'); return false;" title="Tambah Data Manrisk">
	                ...
	                if (empty($html)) {
	                    $html = '<tr><td class="text-center" colspan="31">Data masih kosong!</td></tr>';
	    ...
	    if ($tipe_renstra == 'tujuan_sasaran') {
	        ...
	        foreach ($indikator_list as $id_indikator) {
	            ...
	            if ($data['active'] == 1) {
                    ...
                    $existing_verifikator = $wpdb->get_var($wpdb->prepare("
				        SELECT COUNT(*)
				        FROM data_keterangan_verifikator
				        WHERE id_manrisk = %d
				            AND tipe = %d
				            ...
				            AND tahun_anggaran = %d
				            AND active = 1
				    ", $id_sebelum, $tipe, $id_skpd, $tahun_anggaran));

				    if ($existing_verifikator > 0) {
				        return;
				    }
				    ...
				    $wpdb->insert(
				        ...
				        array('%d', '%d', '%s', '%d', '%d', '%d', '%d', '%d', '%s')
				    );

				    if (!$wpdb->insert_id) {
				        error_log("Error inserting into data_keterangan_verifikator " . "(id_manrisk={$id_sebelum}, tipe={$tipe}): " . $wpdb->last_error
				        );
				    }
		    ...
		    $indikator_data = $wpdb->get_results($wpdb->prepare("
		        ...
		        WHERE id_unik = %s 
		            ...
		            AND active = 1
		    ", $data['id_unik']), ARRAY_A);
		    $indikator_list = !empty($indikator_data) ? array_column($indikator_data, 'id_unik_indikator') : array();
		    
		    if (empty($indikator_list)) {
		        $wpdb->query($wpdb->prepare("
		            UPDATE data_tujuan_sasaran_manrisk_sebelum_pemda 
		            SET active = 0 
		            WHERE id_tujuan_sasaran = %s 
		                AND tipe = %d 
		                AND tahun_anggaran = %d 
		                AND id_jadwal = %d
		        ", $data['id_unik'], $tipe, $tahun_anggaran, $id_jadwal));
		    } else {
		        $placeholders = implode(',', array_fill(0, count($indikator_list), '%s'));
		        $wpdb->query($wpdb->prepare("
		            UPDATE data_tujuan_sasaran_manrisk_sebelum_pemda 
		            SET active = 0 
		            WHERE id_tujuan_sasaran = %s 
		                ...
		                AND tahun_anggaran = %d 
		                AND id_jadwal = %d
		                AND id_indikator NOT IN ($placeholders)
		        ", array_merge([$data['id_unik'], $tipe, $tahun_anggaran, $id_jadwal], $indikator_list)));
	        ...
	        if (!empty($master_data_ind) && $data['active'] == 1) {
	            ...
	            foreach ($indikator_data as $index => $master_item) {
	                $key = $index + 1;
	            ...
	            $kode = $indikator_data[0]['kode_sbl'];
	            
	            $get_id_indikator = array();
	            foreach ($existing_manrisk_data as $manrisk_item) {
	                if ($manrisk_item['active'] == 0) continue; 
	                ...
	                foreach ($grouped_master as $kode_ind => $master_item) {
	                	$existing_verifikator = $wpdb->get_var($wpdb->prepare("
                            ...
                            WHERE id_manrisk = %d
                                ...
                                AND tipe = 3
                                AND active = 1
                        ", $manrisk_item['id'], $id_skpd, $tahun_anggaran));
                        ...
                        if (!$wpdb->insert_id) {
                            error_log("Error inserting into data_keterangan_verifikator (id_manrisk={$manrisk_item['id']}, tipe=3): " . $wpdb->last_error);
                        }
	                    if ($manrisk_item['capaian_teks'] == $master_item['capaianteks'] &&
	                        ...
	                        $get_id_indikator[] = $manrisk_item['id_indikator'];
	                        break;
	                    }
	                }
	            }
	            
	            foreach ($grouped_master as $kode_indikator => $master_item) {
	                $cek_data = false;
	                
	                if (in_array($kode_indikator, $get_id_indikator)) {
	                    $cek_data = true;
	                    continue;
	                }
	                            ...
	                            if ($cek_data_baru == 0) {
				                    ...
				                    $existing_verifikator = $wpdb->get_var($wpdb->prepare("
								        ...
								        WHERE id_manrisk = %d
								            ...
								            AND tipe = 3
								            AND active = 1
								    ", $id_sebelum, $id_skpd, $tahun_anggaran));
								    ...
								    if (!$wpdb->insert_id) {
								        error_log("Error inserting into data_keterangan_verifikator " . "(id_manrisk={$id_sebelum}, tipe={3}): " . $wpdb->last_error
								        );
								    }
	                ...
	                if (!$cek_data) {
	                    $cek_data_baru = $wpdb->get_var($wpdb->prepare("
	                        ...
	                        WHERE id_program_kegiatan = %s
	                         	...
	                         	AND id_skpd = %d
	                         	AND active = 1
	                    ", $id_program_kegiatan, $kode_indikator, $tipe, $tahun_anggaran, $id_skpd));
	                    ...
	                    if ($cek_data_baru == 0) {
	                        ...
	                        $this->get_data_sesudah($id_sebelum, $data, $kode_indikator, $tipe, $tahun_anggaran, $id_skpd, $id_jadwal, $table_name, $tipe_renstra, $master_item['satuancapaian'], $master_item['targetcapaianteks'], $master_item['capaianteks'], $master_item['targetcapaian']);
	                    }
	...
	public function get_data_sesudah($id_sebelum, $data, $id_indikator, $tipe, $tahun_anggaran, $id_skpd, $id_jadwal, $table_name, $tipe_renstra, $satuan_capaian = '', $target_capaian_teks = '', $capaian_teks = '', $target_capaian = '')
	{
	    ...
	    if ($tipe_renstra == 'tujuan_sasaran') {
		    ...
		    $indikator_exists = $wpdb->get_var($wpdb->prepare("
		        SELECT COUNT(*) 
		        FROM {$table_name}
		        WHERE id_unik = %s 
		            AND id_unik_indikator = %s 
		            AND active = 1
		    ", $data['id_unik'], $id_indikator));
		    
		    $is_active = ($indikator_exists > 0) ? 1 : 0;
  	...
  	public function submit_tujuan_sasaran()
	{
	            ...
	            $cek_verifikasi = $wpdb->get_row(
				    $wpdb->prepare("
				        ...
				        WHERE id_manrisk = %d
				          ...
				          AND active = 1
				        LIMIT 1
				    ", $id_sebelum, $_POST['jenis'], $id_skpd, $tahun_anggaran),
				    ARRAY_A
				);

				if (empty($cek_verifikasi)) {
				    $wpdb->insert('data_keterangan_verifikator', $data_verifikasi);
				} else {
				    $wpdb->update(
				        'data_keterangan_verifikator',
				        $data_verifikasi,
				        array('id' => $cek_verifikasi['id'])
				    );
				}
  	...
  	public function edit_tujuan_sasaran_manrisk_penetapan()
  	{
    	global $wpdb;
      	$ret = array(
          	'status' => 'success',
  	...
  	public function submit_tujuan_sasaran_penetapan()
  	{
	    global $wpdb;
	    $ret = array(
          	'status' => 'success',
						        ...
						        if (!empty($kode_bidang)) {
						            ...
						            if (!isset($program_kegiatan_groups[$kode_bidang][$program_kegiatan_key])) {
						                $program_kegiatan_groups[$kode_bidang][$program_kegiatan_key] = array(
						                    ...
						                    'kode_bidang_urusan' => $kode_bidang,
						                    'indikator' => array()
						                );
						            }
	                ...
	                foreach ($grouped_data as $group) {
	                    ...
	                    foreach ($group['program_kegiatan_groups'] as $program_kegiatan_group) {
	                        ...
	                        foreach ($program_kegiatan_group['indikator'] as $indikator_id => $indikator_data) {
	                            $tampil_indikator_sebelum = true;
	                            $tampil_indikator_sesudah = true;
	                            $indikator_count = count($indikator_data['data']);
	                            
	                            $all_id_manrisk = array();
	                            foreach ($indikator_data['data'] as $data_item) {
	                                $all_id_manrisk[] = $data_item['id'];
	                            }
	                            ...
	                            foreach ($indikator_data['data'] as $index => $data_sebelum) {
	                                ...
	                                if ($is_admin) {
		                                ...
		                                if (!empty($get_data_sesudah)) {
		                                    foreach ($get_data_sesudah as $data_sesudah) {
									            ...
									            $pemilik_resiko_sesudah = isset($data_pemilik_resiko[$data_sesudah['pemilik_resiko']]) ? $data_pemilik_resiko[$data_sesudah['pemilik_resiko']] : '';
									            $sumber_sebab_sesudah = isset($data_sumber_sebab[$data_sesudah['sumber_sebab']]) ? $data_sumber_sebab[$data_sesudah['sumber_sebab']] : '';
									            $pihak_terkena_sesudah = isset($data_pihak_terdampak[$data_sesudah['pihak_terkena']]) ? $data_pihak_terdampak[$data_sesudah['pihak_terkena']] : '';
	                                ...
	                                $add_renstra = '';
									$html .= '<td class="text-center"';
									if ($data_sebelum['status'] == 1 && $index == 0) {
									    $html .= ' rowspan="' . $indikator_count . '"';
									}
									$html .= '>';

									if ($data_sebelum['status'] == 1) {
									    if ($index == 0) {
									        $id_manrisk = '[' . implode(',', $all_id_manrisk) . ']';
									        $html .= '
									            <button class="btn btn-secondary" onclick="mapping_program_kegiatan_manrisk(' . $id_manrisk . ', \'' . $id_program . '\', \'' . $id_indikator . '\', ' . $tipe_target . '); return false;" title="Mapping Data">
									                <span class="dashicons dashicons-update-alt"></span>
									            </button>
									            <button class="btn btn-danger" onclick="hapus_program_kegiatan_manrisk(' . $id_manrisk . ', '. $data_sebelum['status'] .'); return false;" title="Hapus Data Sebelum">
									                <span class="dashicons dashicons-trash"></span>
									            </button>';
									    }
									} else {
										    ...
										    if ($now >= $awal && $now <= $akhir) {
										        ...
										        if ($jenisJadwal == 'penetapan') {
										            ...
										            if ($is_admin) {
										                if ($data_verifikasi['status_verifikasi'] == 2) {
										                    $html .= '
										                        <button class="btn btn-success" onclick="verifikasi_program_kegiatan_manrisk(\'' . $data_sebelum['id'] . '\', ' . 3 . '); return false;" title="Verifikasi">
										                            <span class="dashicons dashicons-yes"></span>
										                        </button>
										                    ';
										                } elseif ($data_verifikasi['status_verifikasi'] == 3) {
										                    $html .= '
										                        <button class="btn btn-primary" onclick="edit_program_kegiatan_manrisk_penetapan(' . $id_sesudah . ', ' . $data_sebelum['id'] . ', \'' . $id_program . '\', \'' . $id_indikator . '\', ' . $tipe_sesudah . ', ' . $data_sebelum['kode_bidang_urusan'] . '); return false;" title="Edit Data">
										                            <span class="dashicons dashicons-edit"></span>
										                        </button>
										                    ';
										                }
										                ...
										                if ($data_verifikasi['status_verifikasi'] != 3 && $data_verifikasi['status_verifikasi'] != 2) {
										                    ...
										                    $html .= '
										                        <button class="btn btn-primary" onclick="edit_program_kegiatan_manrisk(' . $data_sebelum['id'] . ', \'' . $id_program . '\', \'' . $id_indikator . '\', ' . $tipe_target . ', ' . $data_sebelum['kode_bidang_urusan'] . '); return false;" title="Edit Data">
										                            <span class="dashicons dashicons-edit"></span>
										                        </button>
										                    ';
										                    ...
										                    if ($draft) {
										                        $html .= '
										                            <button class="btn btn-info" onclick="usulkan_manrisk(\'' . $data_sebelum['id'] . '\', ' . 3 . '); return false;" title="Usulkan">
										            ...
										            if ($is_admin) {
										                ...
										                if ($data_verifikasi['status_verifikasi'] != 3 && $data_verifikasi['status_verifikasi'] != 2) {
										                    ...
										                    $html .= '
										                        <button class="btn btn-success" onclick="tambah_program_kegiatan_manrisk(\'' . $id_program . '\', \'' . $id_indikator . '\', \'' . $program_kegiatan_group['nama_program_kegiatan'] . '\', \'' . $indikator_data['indikator_text'] . '\', ' . $tipe_target . ', ' . $data_sebelum['kode_bidang_urusan'] . '); return false;" title="Tambah">
										                            <span class="dashicons dashicons-plus"></span>
										                        </button>
										                    ';

										                    $html .= '
										                        <button class="btn btn-primary" onclick="edit_program_kegiatan_manrisk(' . $data_sebelum['id'] . ', \'' . $id_program . '\', \'' . $id_indikator . '\', ' . $tipe_target . ', ' . $data_sebelum['kode_bidang_urusan'] . '); return false;" title="Edit">
										                            <span class="dashicons dashicons-edit"></span>
										                        </button>
										                    ';
										                    ...
										                    if ($indikator_count > 1) {
										                        $html .= '
										                            <button class="btn btn-danger" onclick="hapus_program_kegiatan_manrisk(' . $data_sebelum['id'] . '); return false;" title="Hapus">
										                                <span class="dashicons dashicons-trash"></span>
										                            </button>
										                        ';
										                    }
										                    ...
										                    if ($draft) {
										                        $html .= '
										                            <button class="btn btn-info" onclick="usulkan_manrisk(\'' . $data_sebelum['id'] . '\', ' . $tipe_target . '); return false;" title="Usulkan">
										                                <span class="dashicons dashicons-upload"></span>
										                            </button>
										                        ';
										                    }
	...
	public function edit_program_kegiatan_manrisk_penetapan()
	{
	    global $wpdb;
	    $ret = array(
	        'status' => 'success',
	...
	public function submit_program_kegiatan_penetapan()
	{
	    global $wpdb;
	    $ret = array(
	        'status' => 'success',
	...
	public function hapus_program_kegiatan_manrisk()
	{
	    global $wpdb;
	    $ret = array(
	        'status' => 'success',
	    ...
	    if (!empty($_POST)) {
	        if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
	            ...
	            $id_array = is_array($_POST['id']) ? $_POST['id'] : array($_POST['id']);
	...
	public function mapping_program_kegiatan_manrisk()
	{
	    global $wpdb;
	    $ret = array(
	        'status' => 'success',
	    ...
	    if (!empty($_POST)) {
	        if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
	            ...
	            if (!empty($_POST['all_id'])) {
	                $all_id = json_decode($_POST['all_id'], true);
	                if (!is_array($all_id)) {
	                    $ret['status'] = 'error';
	                    $ret['message'] = 'Format IDs tidak valid!';
	                    wp_send_json($ret);
	                }
	            } else {
	                $ret['status'] = 'error';
	                $ret['message'] = 'IDs kosong!';
	                wp_send_json($ret);
	            }
	            ...
	            $placeholders = implode(',', array_fill(0, count($all_id), '%d'));
	            ...
	            ", array_merge([$id_program_kegiatan, $tahun_anggaran, $id_skpd], $all_id)), ARRAY_A);
	...
	public function submit_mapping_program_kegiatan()
	{
	    global $wpdb;
	    $ret = array(
	        'status' => 'success',
	    ...
	    if (!empty($_POST)) {
	        if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
	            ...
	            if (!empty($_POST['all_id'])) {
	                ...
	                if (!is_array($all_id)) {
	                    $ret['status'] = 'error';
	                    $ret['message'] = 'Format ID tidak valid!';
	                    wp_send_json($ret);
	                }
	            } else {
	                $ret['status'] = 'error';
	            ...
	            $cek_data = array(
	                'uraian_resiko', 'kode_resiko', 'pemilik_resiko', 
	                'uraian_sebab', 'sumber_sebab', 'controllable', 
	                'uraian_dampak', 'pihak_terkena', 'skala_dampak', 
	                'skala_kemungkinan', 'rencana_tindak_pengendalian'
	            );

	            $get_data = false;
	            foreach ($cek_data as $data) {
	                $val = isset($get_data_tujuan[$data]) ? trim($get_data_tujuan[$data]) : '';

	                if (
	                    !empty($val) &&
	                    $val !== '' &&
	                    !(
	                        ($data == 'skala_dampak' && $val == '0') ||
	                        ($data == 'skala_kemungkinan' && $val == '0') ||
	                        ($data == 'controllable' && $val == '2')
	                    )
	                ) {
	                    $get_data = true;
	                    break;
	            ...
	            if ($get_data) {
	                $ret['status'] = 'error';
	                $ret['message'] = 'Data sudah terisi!';
	                wp_send_json($ret);
	            }

	            foreach ($all_id as $index => $id) {
	                ...
	                if ($index == 0) {
	                    $data_update = array(
	                        'uraian_resiko' => $get_data_sumber['uraian_resiko'],
	                        'kode_resiko' => $get_data_sumber['kode_resiko'],
	                        'pemilik_resiko' => $get_data_sumber['pemilik_resiko'],
	                        'uraian_sebab' => $get_data_sumber['uraian_sebab'],
	                        'sumber_sebab' => $get_data_sumber['sumber_sebab'],
	                        'controllable' => $get_data_sumber['controllable'],
	                        'uraian_dampak' => $get_data_sumber['uraian_dampak'],
	                        'pihak_terkena' => $get_data_sumber['pihak_terkena'],
	                        'skala_dampak' => $get_data_sumber['skala_dampak'],
	                        'skala_kemungkinan' => $get_data_sumber['skala_kemungkinan'],
	                        'rencana_tindak_pengendalian' => $get_data_sumber['rencana_tindak_pengendalian']
	                    );
	                    ...
	                    $data_insert = array(
	                        'id_program_kegiatan' => $get_data_tujuan['id_program_kegiatan'],
	                        'id_indikator' => $get_data_tujuan['id_indikator'],
	                        'tipe' => $get_data_tujuan['tipe'],
	                        'uraian_resiko' => $get_data_sumber['uraian_resiko'],
	                        'kode_resiko' => $get_data_sumber['kode_resiko'],
	                        'pemilik_resiko' => $get_data_sumber['pemilik_resiko'],
	                        'uraian_sebab' => $get_data_sumber['uraian_sebab'],
	                        ...
	                        'skala_kemungkinan' => $get_data_sumber['skala_kemungkinan'],
	                        'rencana_tindak_pengendalian' => $get_data_sumber['rencana_tindak_pengendalian'],
	                        'capaian_teks' => $get_data_tujuan['capaian_teks'],
	                        'satuan_capaian' => $get_data_tujuan['satuan_capaian'],
	                        'target_capaian_teks' => $get_data_tujuan['target_capaian_teks'],
	                        'target_capaian' => $get_data_tujuan['target_capaian'],
	                        'id_skpd' => $get_data_tujuan['id_skpd'],
	                        'tahun_anggaran' => $get_data_tujuan['tahun_anggaran'],
	                        'active' => 1
	                    );

	                    $wpdb->insert('data_program_kegiatan_manrisk_sebelum', $data_insert);
	                }
	            ...
	            $ret['message'] = 'Berhasil mapping data manrisk!';
	        ...
	        if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
	            ...
	            if (!empty($get_data_tujuan)) {
	                foreach ($get_data_tujuan as $tujuan) {
	                    ...
	                    $indikator_tujuan_array = array();
	                    if (!empty($get_indikator_tujuan)) {
	                        foreach ($get_indikator_tujuan as $ind) {
	                            if (!in_array($ind['indikator_teks'], $indikator_tujuan_array)) {
	                                $indikator_tujuan_array[] = $ind['indikator_teks'];
	                            }
	                        }
	                    }
	                    ...
	                    $get_data_sasaran = $wpdb->get_results($wpdb->prepare("
	                        ...
	                        WHERE id_jadwal = %d
	                            ...
	                            AND id_unik_indikator IS NULL
	                            AND active = 1
	                    ", $id_jadwal, $id_skpd, $tujuan['id_unik']), ARRAY_A);
	                    ...
	                    if (!empty($get_data_sasaran)) {
	                        foreach ($get_data_sasaran as $sasaran) {
	                            ...
	                            $indikator_sasaran_array = array();
	                            if (!empty($get_indikator_sasaran)) {
	                                foreach ($get_indikator_sasaran as $ind) {
	                                    if (!in_array($ind['indikator_teks'], $indikator_sasaran_array)) {
	                                        $indikator_sasaran_array[] = $ind['indikator_teks'];
	                                    }
	                                }
	                            }
	                            ...
	                            ", $id_jadwal, $id_skpd, $sasaran['id_unik']), ARRAY_A);
	                            ...
	                            if (!empty($get_data_program)) {
	                                foreach ($get_data_program as $program) {
	                                    ...
	                                    if (!isset($grouped_data[$tujuan_key])) {
	                                        $grouped_data[$tujuan_key] = array(
	                                            'indikator_tujuan' => $indikator_tujuan_array,
	                                            'sasaran' => array()
	                                        );
	                                    }
	                                    
	                                    if (!isset($grouped_data[$tujuan_key]['sasaran'][$sasaran_key])) {
	                                        $grouped_data[$tujuan_key]['sasaran'][$sasaran_key] = array(
	                                            'indikator_sasaran' => $indikator_sasaran_array,
	                                            'program' => array()
	                                        );
	                                    }
	                                    ...
	                                    if (!empty($get_indikator_program)) {
	                                        foreach ($get_indikator_program as $ind) {
	                                            if (!in_array($ind['indikator'], $grouped_data[$tujuan_key]['sasaran'][$sasaran_key]['program'][$program_key]['indikator'])) {
	                                                $grouped_data[$tujuan_key]['sasaran'][$sasaran_key]['program'][$program_key]['indikator'][] = $ind['indikator'];
	                                            }
	                                        }
	                                    }
	                                }
	                ...
	                foreach ($grouped_data as $tujuan_text => $tujuan_data) {
	                    ...
	                    foreach ($tujuan_data['sasaran'] as $sasaran_text => $sasaran_data) {
	                        ...
	                        uasort($sasaran_data['program'], function($a, $b) {
	                            $kode_a = $a['kode_bidang'];
	                            $kode_b = $b['kode_bidang'];
	                            
	                            $is_a_x = preg_match('/^X\./', $kode_a);
	                            $is_b_x = preg_match('/^X\./', $kode_b);
	...
	public function get_table_tujuan_sasaran_pemda()
	{
	    global $wpdb;
	    $ret = array(
	        'status' => 'success',
	                ...
	                foreach ($data_jadwal_list as $jadwal) {
	                    ...
	                    if (!empty($get_data)) {
	                        ...
	                        foreach ($get_data as $row) {
	                            ...
	                            if (isset($row['tipe']) && $row['tipe'] == 0) {
	                                if ($jadwal['jenis_jadwal'] == 'rpjmd') {
	                                    $nama_table = "data_rpjmd_tujuan{$prefix_history}";
	                                } else if ($jadwal['jenis_jadwal'] == 'rpd') {
	                                    $nama_table = "data_rpd_tujuan{$prefix_history}";
	                                }
	                                ...
	                                $get_data_tujuan = $wpdb->get_row($wpdb->prepare("
	                                    ...
	                                    WHERE id_unik = %s
	                                        AND id_jadwal = %d
	                                        AND active = 1
	                                ", $row['id_tujuan_sasaran'], $id_jadwal), ARRAY_A);
	                                ...
	                                if (!empty($get_data_tujuan)) {
	                                    ...
	                                    $get_data_indikator = $wpdb->get_row($wpdb->prepare("
	                                        ...
	                                        WHERE id_unik = %s
	                                            ...
	                                            AND id_jadwal = %d
	                                            AND active = 1
	                                    ", $get_data_tujuan['id_unik'], $row['id_indikator'], $id_jadwal), ARRAY_A);
	                                    
	                                    $row['indikator'] = !empty($get_data_indikator) ? $get_data_indikator['indikator_teks'] : '';
	                                    $row['label_tipe'] = 'Tujuan Pemda:';
	                                }

	                            } elseif (isset($row['tipe']) && $row['tipe'] == 1) {
	                                if ($jadwal['jenis_jadwal'] == 'rpjmd') {
	                                    $nama_table = "data_rpjmd_sasaran{$prefix_history}";
	                                } else if ($jadwal['jenis_jadwal'] == 'rpd') {
	                                    $nama_table = "data_rpd_sasaran{$prefix_history}";
	                                }
	                                ...
	                                if (!empty($get_data_sasaran)) {
	                                    ...
	                                    $get_data_indikator = $wpdb->get_row($wpdb->prepare("
	                                        ...
	                                        WHERE id_unik = %s
	                                            ...
	                                            AND id_jadwal = %d
	                                            AND active = 1
	                                    ", $get_data_sasaran['id_unik'], $row['id_indikator'], $id_jadwal), ARRAY_A);

	                                    $row['indikator'] = !empty($get_data_indikator) ? $get_data_indikator['indikator_teks'] : '';
	                                    $row['label_tipe'] = 'Sasaran Pemda:';
	                                }
	                            }
	                            ...
	                            if (!isset($tujuan_sasaran_groups[$tujuan_sasaran_key])) {
	                                $tujuan_sasaran_groups[$tujuan_sasaran_key] = array(
	                                    'nama_tujuan_sasaran' => $nama_tujuan_sasaran,
	                                    'label_tipe' => $row['label_tipe'],
	                                    'tipe' => $row['tipe'],
	                                    'id_tujuan_sasaran' => $row['id_tujuan_sasaran'],
	                            ...
	                            $indikator_key = $row['id_indikator'];
	                            if (!isset($tujuan_sasaran_groups[$tujuan_sasaran_key]['indikator'][$indikator_key])) {
	                                $tujuan_sasaran_groups[$tujuan_sasaran_key]['indikator'][$indikator_key] = array(
	                                    'indikator_text' => $row['indikator'],
	                                    'data' => array()
	                                );
	                            }
	                            
	                            $tujuan_sasaran_groups[$tujuan_sasaran_key]['indikator'][$indikator_key]['data'][] = $row;
	                        }

	                        $no = 1;
	                        
	                        uasort($tujuan_sasaran_groups, function($a, $b) {
	                            return $a['tipe'] <=> $b['tipe'];
	                        });
	                          
	                        foreach ($tujuan_sasaran_groups as $tujuan_sasaran_group) {
	                            $tampil_data_tujuan_sasaran = true;
	                            $tampil_data_tujuan_sasaran_sesudah = true;
	                            ...
	                            foreach ($tujuan_sasaran_group['indikator'] as $indikator_id => $indikator_data) {
	                                ...
	                                foreach ($indikator_data['data'] as $index => $data_sebelum) {
	                                    ...
	                                    $html .= '
	                                        <td class="text-center">
	                                            <button class="btn btn-primary" onclick="tambah_tujuan_sasaran_manrisk_pemda( \'' . $id_tujuan . '\', \'' . $id_indikator . '\', \'' . $tujuan_sasaran_group['nama_tujuan_sasaran'] . '\', \'' . $indikator_data['indikator_text'] . '\', ' . $tipe_target . ', ' . $id_jadwal . '); return false;" title="Tambah Data Manrisk">
	                                                <span class="dashicons dashicons-plus"></span>
	                                            </button>
	                                            <button class="btn btn-warning" onclick="edit_tujuan_sasaran_manrisk_pemda(' . $data_sebelum['id'] . ', \'' . $id_tujuan . '\', \'' . $id_indikator . '\', ' . $tipe_target . ', ' . $id_jadwal . '); return false;" title="Edit Data Sebelum">
	                                                <span class="dashicons dashicons-edit"></span>
	                                            </button>
	                                            <button class="btn btn-success" onclick="verif_tujuan_sasaran_manrisk_pemda(' . $id_sesudah . ', ' . $data_sebelum['id'] . ', \'' . $id_tujuan . '\', \'' . $id_indikator . '\', ' . $tipe_sesudah . ', ' . $id_jadwal . '); return false;" title="Verifikasi Data">
	                                                <span class="dashicons dashicons-yes"></span>
	                                            </button>
	                                            <button class="btn btn-danger" onclick="hapus_tujuan_sasaran_manrisk_pemda(' . $data_sebelum['id'] . '); return false;" title="Hapus Data Sebelum">
	                                                <span class="dashicons dashicons-trash"></span>
	                                            </button>
	                                        </td>
	                                    ';
	    ...
	    if (!empty($_POST)) {
	        if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
	            ...
	            $data = array(
	                ...
	                'uraian_resiko'                 => $_POST['uraian_resiko'],
	                'kode_resiko'                   => $_POST['kode_resiko'],
	                'pemilik_resiko'                => intval($_POST['pemilik_resiko']),
	                'uraian_sebab'                  => $_POST['uraian_sebab'],
	                'sumber_sebab'                  => intval($_POST['sumber_sebab']),
	                'controllable'                  => $controllable_status, 
	                'uraian_dampak'                 => $_POST['uraian_dampak'],
	                'pihak_terkena'                 => intval($_POST['pihak_terkena']),
	                'skala_dampak'                  => $skala_dampak,
	                'skala_kemungkinan'             => $skala_kemungkinan,
	                'rencana_tindak_pengendalian'   => $_POST['rencana_tindak_pengendalian'],
	                'id_jadwal'                     => $id_jadwal,
	            ...
	            if ($id <= 0) {
	                ...
	                $wpdb->insert('data_tujuan_sasaran_manrisk_sebelum_pemda', $data);
	            } else {
	                $wpdb->update('data_tujuan_sasaran_manrisk_sebelum_pemda', 
	                    $data, 
	                    array('id' => $id)
	                );
	            }
	...
	public function edit_tujuan_sasaran_manrisk_pemda()
	{
	    global $wpdb;
	    $ret = array(
	        'status' => 'success',
	        ...
	        if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
	            ...
	            $get_data = $wpdb->get_row($wpdb->prepare("
	                ...
	                WHERE id = %d
	                    ...
	                    AND active = 1
	                LIMIT 1
	            ", $id, $_POST['id_tujuan_sasaran'], $_POST['id_indikator'], $id_jadwal, $tahun_anggaran, $tipe), ARRAY_A);

	            $nama_tujuan_sasaran = '';
	            $indikator_teks = '';

	            if (!empty($get_data)) {
	                $data_jadwal = $wpdb->get_row($wpdb->prepare("
	                    SELECT 
	                    	jenis_jadwal, 
	                    	relasi_perencanaan
	                    FROM data_jadwal_lokal 
	                ...
	                if ($data_jadwal) {
	                    if ($data_jadwal['jenis_jadwal'] == 'rpjmd') {
	                        $table_name = ($tipe == 0) ? "data_rpjmd_tujuan{$prefix_history}" : "data_rpjmd_sasaran{$prefix_history}";
	                    } else if ($data_jadwal['jenis_jadwal'] == 'rpd') {
	                        $table_name = ($tipe == 0) ? "data_rpd_tujuan{$prefix_history}" : "data_rpd_sasaran{$prefix_history}";
	                    }
	                }
	                ...
	                if ($tipe == 0) {
	                    $get_tujuan = $wpdb->get_row($wpdb->prepare("
	                        ...
	                        WHERE id_unik = %s
	                            ...
	                            AND active = 1
	                        LIMIT 1
	                    ", $get_data['id_tujuan_sasaran'], $id_jadwal), ARRAY_A);
	                    ...
	                    $get_indikator = $wpdb->get_row($wpdb->prepare("
	                        ...
	                        WHERE id_unik = %s
	                            ...
	                            AND id_jadwal = %d
	                            AND active = 1
	                    ", $get_data['id_tujuan_sasaran'], $get_data['id_indikator'], $id_jadwal), ARRAY_A);
	...
	public function verif_tujuan_sasaran_manrisk_pemda()
	{
	    global $wpdb;
	    $ret = array(
	        'status' => 'success',
	    ...
	    if (!empty($_POST)) {
	        if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
	            ...
	            if (!empty($_POST['id'])) {
	                $get_data = $wpdb->get_row($wpdb->prepare("
	                    ...
	                    WHERE id = %d
	                        ...
	                        AND active = 1
	                    LIMIT 1
	                ", $_POST['id'], $_POST['id_sebelum'], $_POST['id_tujuan_sasaran'], $_POST['id_indikator'], $id_jadwal, $tahun_anggaran, $tipe), ARRAY_A);
	            ...
	            if ($tipe == 0) {
	                $get_tujuan = $wpdb->get_row($wpdb->prepare("
	                    ...
	                    WHERE id_unik = %s
	                        ...
	                        AND active = 1
	                    LIMIT 1
	                ", $id_tujuan_sasaran, $id_jadwal), ARRAY_A);
	                ...
	                $get_indikator = $wpdb->get_row($wpdb->prepare("
	                    ...
	                    WHERE id_unik = %s
	                        ...
	                        AND id_jadwal = %d
	                        AND active = 1
	                ", $id_tujuan_sasaran, $id_indikator, $id_jadwal), ARRAY_A);
	            ...
	            $get_data_sebelum = null;
	            if (!empty($_POST['id_sebelum'])) {
	                $get_data_sebelum = $wpdb->get_row($wpdb->prepare("
	                    SELECT 
	                    	* 
	                    FROM data_tujuan_sasaran_manrisk_sebelum_pemda
	                    WHERE id = %d
	                        ...
	                        AND active = 1
	                    LIMIT 1
	                ", $_POST['id_sebelum'], $id_jadwal, $tahun_anggaran), ARRAY_A);
	            }
	...
	public function submit_verif_tujuan_sasaran_pemda()
	{
	    global $wpdb;
	    $ret = array(
	        'status' => 'success',
	    ...
	    if (!empty($_POST)) {
	        if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
	            ...
	            $data = array(
	                ...
	                'id_indikator'                  => $_POST['id_indikator'],
	                'tipe'                          => $tipe,
	                'id_sebelum'                    => intval($_POST['id_sebelum']),
	                'uraian_resiko'                 => $_POST['uraian_resiko'],
	                'kode_resiko'                   => $_POST['kode_resiko'],
	                'pemilik_resiko'                => intval($_POST['pemilik_resiko']),
	                'uraian_sebab'                  => $_POST['uraian_sebab'],
	            ...
	            if ($id <= 0) {
	                $data['created_at'] = current_time('mysql');
	                $wpdb->insert('data_tujuan_sasaran_manrisk_sesudah_pemda', $data);
	            } else {
	                $wpdb->update('data_tujuan_sasaran_manrisk_sesudah_pemda',
	                    $data, 
	                    array('id' => $id)
	                );
	            }
	...
	public function hapus_tujuan_sasaran_manrisk_pemda()
	{
	    global $wpdb;
	    $ret = array(
	        'status' => 'success',
	...
	public function usulkan_manrisk()
	{
		global $wpdb;
		$ret = array(
			'status' => 'success',
		...
		if (!empty($_POST)) {
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
				if (!empty($_POST['id_manrisk'])) {
					$id_manrisk = $_POST['id_manrisk'];
				} else {
					$ret['status'] = 'error';
					$ret['message'] = 'Id kosong!';
				}
					...
					$data_terverifikasi = $wpdb->get_row(
						$wpdb->prepare("
							...
							WHERE id_manrisk = %d
								...
								AND active=1
								AND tahun_anggaran=%d
						", $id_manrisk, $tipe, $tahun_anggaran),
						ARRAY_A
					);
	...
	public function get_verifikasi_usulan_by_id()
	{
		global $wpdb;
		$ret = array(
			'status' => 'success',
			...
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
				if (!empty($_POST['id'])) {
					if (!empty($_POST['jenis'])) {
						$jenis = $_POST['jenis'];
						// untuk mengatur tabel sesuai tipe usulan
						$nama_tabel = array(
							"tujuan_sasaran" => "data_tujuan_sasaran_manrisk_sebelum",
							"program"	=> "data_program_kegiatan_manrisk_sebelum"
						);

						$data = $wpdb->get_row(
							$wpdb->prepare("
								SELECT *
								FROM $nama_tabel[$jenis]
								WHERE id = %d
									AND active=1
							", $_POST['id']),
							ARRAY_A
	...
	public function submit_verifikasi_usulan()
	{
	    global $wpdb;
	    $ret = array(
	        'status' => 'success',
	        'message' => 'Berhasil Verifikasi Usulan!'
	    );
	    ...
	    if (!empty($_POST)) {
	        if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
	            ...
	            if (empty($_POST['id_usulan'])) {
	                $ret['status'] = 'error';
	                $ret['message'] = 'Id Usulan kosong!';
	            } elseif (empty($_POST['idSkpd'])) {
	                $ret['status'] = 'error';
	                $ret['message'] = 'Id Perangkat Daerah kosong!';
	            } elseif (empty($_POST['keterangan'])) {
	                $ret['status'] = 'error';
	                $ret['message'] = 'Keterangan kosong!';
	            } elseif (empty($_POST['tahunAnggaran'])) {
	                $ret['status'] = 'error';
	                $ret['message'] = 'Tahun Anggaran kosong!';
	            } elseif (empty($_POST['verifikasi_usulan'])) {
	                $ret['status'] = 'error';
	                $ret['message'] = 'Status Verifikasi kosong!';
	            } elseif (empty($_POST['jenis'])) {
	                $ret['status'] = 'error';
	                $ret['message'] = 'Jenis Usulan kosong!';
	            }
	            ...
	            $id_usulan = intval($_POST['id_usulan']);
	            $tipe_usulan = isset($_POST['tipe_usulan']) ? intval($_POST['tipe_usulan']) : 0;
	            $idSkpd = intval($_POST['idSkpd']);
	            $keterangan = sanitize_text_field($_POST['keterangan']);
	            $tahunAnggaran = intval($_POST['tahunAnggaran']);
	            $verifikasi_usulan = sanitize_text_field($_POST['verifikasi_usulan']);
	            ...
	            if (!array_key_exists($verifikasi_usulan, $status_map)) {
	                $ret['status'] = 'error';
	                $ret['message'] = 'Nilai Verifikasi tidak valid!';
	                wp_send_json($ret);
	            }

	            $input_verifikasi = $status_map[$verifikasi_usulan];
	            ...
	            if (!array_key_exists($jenis, $nama_tabel)) {
	                $ret['status'] = 'error';
	                $ret['message'] = 'Jenis Usulan tidak valid!';
	                wp_send_json($ret);
	            }

	            $tabel = $nama_tabel[$jenis];
	            $data_usulan = $wpdb->get_row(
	                $wpdb->prepare("
	                	SELECT * 
	                	FROM `$tabel` 
	                	WHERE id = %d 
	            ...
	            if (empty($data_usulan)) {
	                $ret['status'] = 'error';
	                $ret['message'] = 'Data Usulan tidak ditemukan!';
	                wp_send_json($ret);
	            }

	            $data_terverifikasi = $wpdb->get_row(
	                $wpdb->prepare("
	                	SELECT * FROM data_keterangan_verifikator
	                    WHERE id_manrisk = %d
	                       AND tipe = %d
	                       AND id_skpd = %d
	                       AND tahun_anggaran = %d
	            ...
	            if (empty($data_terverifikasi)) {
	                ...
	                $wpdb->insert('data_keterangan_verifikator', $insert_data);
	                
	                if (!$wpdb->insert_id) {
	                    $ret['status'] = 'error';
	                    $ret['message'] = 'Gagal menyimpan data ke database!';
	                }
	            } else {
	                $data_keterangan = json_decode($data_terverifikasi['keterangan_verifikasi'], true);

	                if (json_last_error() === JSON_ERROR_NONE && is_array($data_keterangan)) {
	                    $keterangan_lama = isset($data_keterangan['keterangan_lama']) && is_array($data_keterangan['keterangan_lama']) 
	                        ? $data_keterangan['keterangan_lama'] 
	                        : array();
	                    
	                    if (isset($data_keterangan['keterangan_baru'])) {
	                        array_unshift($keterangan_lama, $data_keterangan['keterangan_baru']);
	                    }
	                } else {
	                    $keterangan_lama = !empty($data_terverifikasi['keterangan_verifikasi']) ? array(array("tanggal" => null, "pesan" => $data_terverifikasi['keterangan_verifikasi'])) : array();
	                }
	                ...
	                if ($wpdb->rows_affected === 0 && $wpdb->last_error) {
	                    $ret['status'] = 'error';
	                    $ret['message'] = 'Gagal memperbarui data ke database: ' . $wpdb->last_error;
	                }
	            }
