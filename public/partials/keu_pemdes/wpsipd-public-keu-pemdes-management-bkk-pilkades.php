...
<div class="cetak">
    ...
    <h1 class="text-center" style="margin:3rem;">Manajemen Data BKK Pemilihan Kepala Desa</h1>
        <div style="margin-bottom: 25px;">
            <button class="btn btn-primary" onclick="tambah_data_bkk_pilkades();"><i class="dashicons dashicons-plus"></i> Tambah Data</button>
        </div>
...
<div class="modal fade mt-4" id="modalTambahDataBKKPilkades" tabindex="-1" role="dialog" aria-labelledby="modalTambahDataBKKPilkadesLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalTambahDataBKKPilkadesLabel">Data BKK Pemilihan Kepala Desa</h5>
            ...
            <div class="modal-footer">
                <button class="btn btn-primary submitBtn" onclick="submitTambahDataFormBKKPilkades()">Simpan</button>
...
function get_data_bkk_pilkades(){
    if(typeof databkk_pilkades == 'undefined'){
        window.databkk_pilkades = jQuery('#management_data_table').on('preXhr.dt', function(e, settings, data){
            ...
            "ajax": {
                ...
                dataType: 'json',
                data:{
                    'action': 'get_datatable_bkk_pilkades',
...
function hapus_data(id){
    ...
    if(confirmDelete){
        ...
        jQuery.ajax({
            ...
            type:'post',
            data:{
                'action' : 'hapus_data_bkk_pilkades_by_id',
                'api_key': '<?php echo get_option( '_crb_api_key_extension' ); ?>',
                'id'     : id
            },
            dataType: 'json',
...
function edit_data(_id){
    ...
    jQuery.ajax({
        ...
        dataType: 'json',
        data:{
            'action': 'get_data_bkk_pilkades_by_id',
            'api_key': '<?php echo get_option( '_crb_api_key_extension' ); ?>',
            'id': _id,
        },
...
//show tambah data
function tambah_data_bkk_pilkades(){
    jQuery('#id_data').val('');
...
function submitTambahDataFormBKKPilkades(){
    ...
    jQuery.ajax({
        ...
        dataType: 'json',
        data:{
            'action': 'tambah_data_bkk_pilkades',
            'api_key': '<?php echo get_option( '_crb_api_key_extension' ); ?>',
            'id_data': id_data,
        ...
        success: function(res){
            alert(res.message);
            jQuery('#modalTambahDataBKKPilkades').modal('hide');
            if(res.status == 'success'){
                get_data_bkk_pilkades();
            }else{
                jQuery('#wrap-loading').hide();
