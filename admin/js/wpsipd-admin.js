...
function run_url(that) {
	var url = jQuery(that).attr('href');
	var id_skpd = [];
	jQuery(that).closest('.body-tahun').find('ul li input[type="checkbox"]').map(function (i, b) {
		if (jQuery(b).is(':checked')) {
			id_skpd.push(jQuery(b).attr('value'));
		}
	});
	url += '&id_skpd=' + id_skpd.join(',');
...
jQuery(document).ready(function () {
	...
	if (jQuery("#body_label").length >= 1) {
		...
		jQuery('#tambah_label_komponen').on('click', function () {
			...
			var rencana_pagu_label = jQuery('#rencana_pagu_label').val();
				...
				if (confirm("Apakah anda yakin akan menyimpan data ini!")) {
					...
					jQuery.ajax({
						...
						data: {
							...
							"rencana_pagu_label": rencana_pagu_label,
		...
		jQuery('#body_label').on('click', '.edit-label', function () {
			...
			data_label_komponen.map(function (b, i) {
				if (b.id == id_label) {
					...
					jQuery('#rencana_pagu_label').val(b.rencana_pagu);
	...
	jQuery('body').on('click', '.select-all', function () {
		jQuery(this).closest('.body-tahun').find('input[type="checkbox"]').prop('checked', jQuery(this).is(':checked'));
	});
});