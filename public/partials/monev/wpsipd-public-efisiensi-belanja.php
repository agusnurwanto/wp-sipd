...
if (!empty($data_anggaran)) {
	...
	foreach ($data_anggaran as $v_anggaran) {
		...
		if (empty($all_unit[$v_anggaran['id_sub_skpd']])) {
			die(print_r($v_anggaran));
		}
	...
	foreach ($data_all_unit as $id_sub_skpd => $data) {
		...
		$title = 'Detail Efisiensi Belanja | ' . $input['tahun_anggaran'];
	...
	foreach ($rek_all as $kode_akun => $data) {
		...
		$title = 'Detail Efisiensi Belanja Pemerintah Daerah | ' . $input['tahun_anggaran'];
		$shortcode = '[detail_efisiensi_belanja_pemda tahun_anggaran="' . $input['tahun_anggaran'] . '"]';
