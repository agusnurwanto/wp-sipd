...
$akun_hibah_uang = $wpdb->get_results('SELECT id_akun, kode_akun, nama_akun FROM `data_akun` where is_bankeu_umum=1 and tahun_anggaran='.$input['tahun_anggaran'].' order by kode_akun ASC', ARRAY_A);
...
$akun_hibah_brg = $wpdb->get_results('SELECT id_akun, kode_akun, nama_akun FROM `data_akun` where is_bankeu_khusus=1 and tahun_anggaran='.$input['tahun_anggaran'].' order by kode_akun ASC', ARRAY_A);
...
<div id="cetak" title="Laporan APBD PENJABARAN Lampiran 5 Tahun Anggaran <?php echo $input['tahun_anggaran']; ?>" style="padding: 5px;">
    ...
    <h4 style="text-align: center; font-size: 13px; margin: 10px auto; min-width: 450px; max-width: 570px; font-weight: bold;">DAFTAR NAMA CALON PENERIMA, ALAMAT DAN BESARAN<br>ALOKASI BANTUAN KEUANGAN BERSIFAT KHUSUS YANG DITERIMA SERTA SKPD PEMBERI BANTUAN KEUANGAN<br>TAHUN ANGGARAN <?php echo $input['tahun_anggaran']; ?></h4>
...
<script type="text/javascript">
    ...
    if(type && type=='pergeseran'){
        var extend_action = '<a class="button button-primary" target="_blank" href="'+_url+'" style="margin-left: 10px;">Print APBD Lampiran 5</a>';
    }else{
