                ...
                if($tipe == 'submit_pencairan'){
                    ...
                    if($ret['status'] != 'error'){
                        $wpdb->update($table, array(
                            'status' => 2, // diburah jadi diverifikasi admin
                            'update_at' => current_time('mysql')
                        ), array(
                            'id' => $_POST['id']
                        ));
                    }
                }else if($tipe == 'verikasi_admin'){
                    if (in_array("administrator", $user_meta->roles)){
                        if($jenis_belanja == 'bkk'){
                            $table = 'data_pencairan_bkk_desa';
                        }else if($jenis_belanja == 'bhpd'){
                        ...
                        if($ret['status'] != 'error'){
                            $opsi = array(
                                'status_ver_total' => $_POST['status_pagu'],
                                'ket_ver_total' => $_POST['keterangan_status_pagu'],
                                'status_ver_proposal' => $_POST['status_file'],
                                'ket_ver_proposal' => $_POST['keterangan_status_file'],
                                'status' => 0, // status dirubah jadi menunggu submit
                                'update_at' => current_time('mysql')
                            );
                            if(
                                $_POST['status_pagu'] == 1
                                && empty($_POST['keterangan_status_pagu'])
                            ){
                                $ret = array(
                                    'status' => 'error',
                                    'message'   => 'Keterangan status pagu tidak boleh kosong!'
                                );
                            }else if(
                                $_POST['status_file'] == 1
                                && empty($_POST['keterangan_status_file'])
                            ){
                                $ret = array(
                                    'status' => 'error',
                                    'message'   => 'Keterangan status file tidak boleh kosong!'
                                );
                            }else{
                                if(
                                    $_POST['status_pagu'] == 1
