...
class Wpsipd_Public extends Wpsipd_Public_Base_1
{
	...
	public function jadwal_tagging_rincian_belanja($atts)
	{
		// untuk disable render shortcode di halaman edit page/post
		if (!empty($_GET) && !empty($_GET['post'])) {
			return '';
		}
		require_once plugin_dir_path(dirname(__FILE__)) . 'public/partials/penjadwalan/wpsipd-public-jadwal-tagging-rincian-belanja.php';
	}
			...
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
				if (!empty($_POST['sumber']) && $_POST['sumber'] == 'ri') {
					...
					foreach ($data['detail'] as $i => $v) {
						$cek_id = $wpdb->get_var("SELECT id from data_spp_sipd_ri_detail where tahun_anggaran=" . $_POST["tahun_anggaran"] . " AND id_spp=" . $_POST['id_spp'] . " AND id_skpd=" . $_POST['idSkpd'] . " AND jumlah=" . $v['jumlah'] . " AND uraian='" . $v['uraian'] . "'");
			...
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
				...
				foreach ($data as $i => $v) {
					...
					$opsi = array(
						...
						"verifikasi_npd_by_name" => $v["verifikasi_npd_by_name"],
						"verifikasi_npd_by_nip" => $v["verifikasi_npd_by_nip"],
						"verifikasi_npd_by_role" => $v["verifikasi_npd_by_role"],
						...
						"validasi_npd_by_name" => $v["validasi_npd_by_name"],
						"validasi_npd_by_nip" => $v["validasi_npd_by_nip"],
						"validasi_npd_by_role" => $v["validasi_npd_by_role"],
						"is_tbp" => $v["is_tbp"],
						"tbp_at" => $v["tbp_at"],
						"tbp_by" => $v["tbp_by"],
						"id_jadwal" => $v["id_jadwal"],
					...
					if (!empty($sqlTipe)) {
						switch ($tipe_perencanaan) {
							...
							case 'monev_renja':
								...
								break;
							case 'tagging_rincian':
								// Tambah Jadwal Tagging Rincian
								if (
									!empty($nama)
									&& !empty($tahun_anggaran)
									&& !empty($jadwal_mulai)
									...
									$return = [
										'status'  => 'success',
										'message' => 'Berhasil Tambah Jadwal Tagging Rincian!',
									];
								} else {
									$return = [
										'status'  => 'error',
				...
				if (in_array("administrator", $user_meta->roles)) {
					...
					if (!empty($sqlTipe)) {
						switch ($tipe_perencanaan) {
							...
							case 'tagging_rincian':
								//Update Tagging Rincian
								if (empty($nama) || empty($jadwal_mulai) || empty($jadwal_selesai)) {
									$return = [
										'status'  => 'error',
										'message' => 'Harap diisi semua, tidak boleh ada yang kosong!'
									];
								...
								$return = [
									'status'  => 'success',
									'message' => 'Berhasil Perbarui Jadwal Tagging Rincian!',
								];
								break;
	...
	public function is_running_jadwal_lokal($id_jadwal_lokal)
	{
		global $wpdb;
		...
		$query = $wpdb->prepare("
			...
			WHERE id_jadwal_lokal = %d
			  ...
			  AND waktu_awal <= %s 
			  AND waktu_akhir >= %s
			", $id_jadwal_lokal, $time_now, $time_now
		);
			...
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
				...
				$sql = $wpdb->prepare("SELECT id FROM data_rpjmd_visi_lokal
						WHERE visi_teks=%s 
							...
							AND tahun_anggaran = %d
							AND status=1
							AND active=1", trim($data['visi_teks']), $_POST['tahun_anggaran']);
			...
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
				...
				$sql = $wpdb->prepare("SELECT id FROM data_rpjmd_visi_lokal
						WHERE visi_teks=%s
							...
							AND status=1
							AND tahun_anggaran = %d
							AND active=1", trim($data['visi_teks']), $data['id_visi'], $_POST['tahun_anggaran']);
		...
		try {
			if (!empty($_POST)) {
				if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
					...
					$id_cek = $wpdb->get_var($wpdb->prepare("SELECT * FROM data_rpjmd_misi_lokal WHERE id_visi=%d AND is_locked = 0 AND status=1 AND active=1 AND tahun_anggaran = %d", $_POST['id_visi'], $_POST['tahun_anggaran']));

					if (!empty($id_cek)) {
						throw new Exception("Visi sudah digunakan oleh misi", 1);
					}

					$wpdb->update('data_rpjmd_visi_lokal', ['active' => 0], [
						'id' => $_POST['id_visi']
					]);
				...
				if ($_POST['type'] == 1) {
					...
					$sql = $wpdb->prepare("
						...
						WHERE id_visi=%d 
						  and active=1 
						  AND tahun_anggaran = %d
						ORDER BY urut_misi", $_POST['id_visi'], $_POST['tahun_anggaran']);
					...
					$id_cek = $wpdb->get_var($wpdb->prepare("
						SELECT id FROM data_rpjmd_misi_lokal
							WHERE misi_teks=%s 
										...
										AND status=1
										AND active=1
								", trim($data['misi_teks']), $data['id_visi'], $_POST['tahun_anggaran']));
					...
					$id_cek = $wpdb->get_var($wpdb->prepare("
						SELECT id FROM data_rpjmd_misi_lokal
							WHERE misi_teks=%s 
									...
									AND status=1
									AND tahun_anggaran = %d
									AND active=1", trim($data['misi_teks']), $data['id_misi'], $_POST['tahun_anggaran']));
		...
		try {
			if (!empty($_POST)) {
				if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
					...
					$wpdb->update('data_rpjmd_misi_lokal', ['active' => 0], [
						'id' => $_POST['id_misi']
					]);
					...
					$sql = $wpdb->prepare("
						SELECT * FROM data_rpjmd_tujuan_lokal
							WHERE id_misi=%d AND
								...
								status=1 AND 
								is_locked = 0 AND 
								active=1 ORDER BY id", $_POST['id_misi'], $_POST['tahun_anggaran']);
					...
					$id_cek = $wpdb->get_var($wpdb->prepare("
						SELECT id FROM data_rpjmd_tujuan_lokal
							WHERE tujuan_teks=%s
										...
										AND status=1
										AND active=1
								", trim($data['tujuan_teks']), $data['id_misi'], $_POST['tahun_anggaran']));
					...
					$id_cek = $wpdb->get_var($wpdb->prepare("
						SELECT id FROM data_rpjmd_tujuan_lokal
							WHERE tujuan_teks=%s
										...
										AND status=1
										AND active=1
								", trim($data['tujuan_teks']), $data['id_tujuan'], $_POST['tahun_anggaran']));
		...
		try {
			if (!empty($_POST)) {
				if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
					...
					$wpdb->update('data_rpjmd_tujuan_lokal', ['active' => 0], [
						'id' => $_POST['id_tujuan']
					]);
						...
						$sql = $wpdb->prepare("
							SELECT * FROM data_rpjmd_tujuan_lokal 
								WHERE 
									...
									tahun_anggaran=%d AND
									status=1 AND 
									active=1", $_POST['id_tujuan'], $_POST['tahun_anggaran']);
					...
					$id_cek = $wpdb->get_var($wpdb->prepare("
						SELECT id FROM data_rpjmd_tujuan_lokal
							WHERE indikator_teks=%s
										...
										AND status=1
										AND active=1
								", trim($data['indikator_teks']), $data['id_tujuan'], $_POST['tahun_anggaran']));
					...
					$id_cek = $wpdb->get_var($wpdb->prepare("
						SELECT id FROM data_rpjmd_tujuan_lokal
							WHERE indikator_teks=%s
										...
										AND status=1
										AND active=1
								", trim($data['indikator_teks']), $data['id_tujuan'], $data['id'], $_POST['tahun_anggaran']));
					...
					$id_cek = $wpdb->get_var($wpdb->prepare("
						SELECT id FROM data_rpjmd_sasaran_lokal
							WHERE sasaran_teks=%s
								...
								AND status=1
								AND active=1
					", trim($data['sasaran_teks']), $data['kode_tujuan'], $data['id_sasaran'], $_POST['tahun_anggaran']));
		...
		try {
			if (!empty($_POST)) {
				if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
					...
					$id_cek = $wpdb->get_var($wpdb->prepare("SELECT * FROM data_rpjmd_program_lokal WHERE kode_sasaran=%s AND is_locked = 0 AND status=1 AND active=1 AND tahun_anggaran = %d", $_POST['kode_sasaran'], $_POST['tahun_anggaran']));
					...
					$wpdb->update('data_rpjmd_sasaran_lokal', ['active' => 0], [
						'id' => $_POST['id_sasaran'],
						'active' => 1
					]);
						...
						$sql = $wpdb->prepare("
							SELECT * FROM data_rpjmd_sasaran_lokal 
								WHERE 
									...
									tahun_anggaran=%d AND 
									status=1 AND 
									active=1", $_POST['id_sasaran'], $_POST['tahun_anggaran']);
						$indikator = $wpdb->get_results($sql, ARRAY_A);
					} else {
						$tahun_anggaran = $_POST['tahun_anggaran'];
						$sql = $wpdb->prepare("
					...
					$id_cek = $wpdb->get_var($wpdb->prepare("
						SELECT id FROM data_rpjmd_sasaran_lokal
							WHERE indikator_teks=%s
										...
										AND status=1
										AND active=1
								", trim($data['indikator_teks']), $data['id_sasaran'], $_POST['tahun_anggaran']));
					...
					$id_cek = $wpdb->get_var($wpdb->prepare("
						SELECT id FROM data_rpjmd_sasaran_lokal
							WHERE indikator_teks=%s
										...
										AND status=1
										AND active=1
								", $data['indikator_teks'], $data['id_sasaran'], $data['id'], $_POST['tahun_anggaran']));
			...
			if (!empty($_POST)) {
				if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
					...
					$dataProgram = $wpdb->get_row($wpdb->prepare("
                        ...
                        WHERE u.tahun_anggaran=" . $_POST['tahun_anggaran'] . " AND id_program=%d
		...
		try {
			if (!empty($_POST)) {
				if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
					...
					$wpdb->update('data_rpjmd_program_lokal', ['active' => 0], [
						'id' => $_POST['id'],
						'active' => 1
					]);
						...
						$sql = $wpdb->prepare("
							SELECT * FROM data_rpjmd_program_lokal 
								WHERE 
									...
									tahun_anggaran=%d AND 
									status=1 AND 
									active=1", $_POST['kode_program'], $_POST['tahun_anggaran']);
						$indikator = $wpdb->get_results($sql, ARRAY_A);
					} else {
						$tahun_anggaran = $_POST['tahun_anggaran'];
						$sql = $wpdb->prepare("
					...
					$id_cek = $wpdb->get_var($wpdb->prepare("
						SELECT id FROM data_rpjmd_program_lokal
							WHERE indikator=%s
										...
										AND status=1
										AND active=1
								", trim($data['indikator_teks']), $data['kode_program'], $_POST['tahun_anggaran']));
			...
			foreach ($units as $key => $unit) {
				...
				$tujuan_all = $wpdb->get_results($wpdb->prepare("
					...
					WHERE id_unit=%d 
					  ...
					  AND tahun_anggaran = %d
					ORDER BY urut_tujuan
				", $unit['id_skpd'], $opt['tahun_anggaran']), ARRAY_A);
				...
				foreach ($tujuan_all as $keyTujuan => $tujuan_value) {
					...
					$sasaran_all = $wpdb->get_results($wpdb->prepare("
						...
						WHERE kode_tujuan=%s 
						  ...
						  AND tahun_anggaran = %d
						ORDER BY urut_sasaran
						", $tujuan_value['id_unik'], $opt['tahun_anggaran']), ARRAY_A);
					...
					foreach ($sasaran_all as $keySasaran => $sasaran_value) {
						...
						$program_all = $wpdb->get_results($wpdb->prepare("
							...
							WHERE kode_sasaran=%s 
							  ...
							  AND active=1 
							ORDER BY id
						", $sasaran_value['id_unik'], $tujuan_value['id_unik'], $opt['tahun_anggaran']), ARRAY_A);
							...
							if (empty($program_value['id_unik_indikator'])) {
								$kegiatan_all = $wpdb->get_results($wpdb->prepare("
									...
									WHERE kode_program=%s 
									  ...
									  AND active=1 
									ORDER BY id
								", $program_value['id_unik'], $sasaran_value['id_unik'], $tujuan_value['id_unik'], $opt['tahun_anggaran']), ARRAY_A);
									...
									if (empty($kegiatan_value['id_unik_indikator'])) {
										$sub_kegiatan_all = $wpdb->get_results($wpdb->prepare("
											...
											WHERE kode_kegiatan=%s 
											  ...
											  AND active=1 
											ORDER BY id
										", $kegiatan_value['id_unik'], $program_value['id_unik'], $sasaran_value['id_unik'], $tujuan_value['id_unik'], $opt['tahun_anggaran']), ARRAY_A);

										if (!empty($sub_kegiatan_all)) {
		...
		if (!empty($_POST)) {
			if (!empty($_POST['api_key']) && $_POST['api_key'] == get_option('_crb_api_key_extension')) {
				if (!empty($_POST['tipe_perencanaan'])) {
					...
					$sqlRec .= ' ORDER BY tahun_anggaran DESC';
	...
	private function process_data_rpjmd_esakip_to_lokal(array $data, int $tahun_anggaran)
    {
        global $wpdb;
        ...
        try {
            ...
            $this->deactivate_existing_rpjmd_lokal($tahun_anggaran);
			
            // 2. Proses secara hirarki dan dapatkan ID mapping
            $visi_id_map = $this->process_upsert_visi_rpjmd_esakip($data['visi'], $tahun_anggaran);
            $misi_id_map = $this->process_upsert_misi_rpjmd_esakip($data['misi'], $tahun_anggaran, $visi_id_map);
            $misi_detail = $this->process_misi_detail_esakip($data['misi_detail']);

            $this->process_upsert_tujuan_rpjmd_esakip($data['tujuan'], $tahun_anggaran, $misi_id_map, $misi_detail);
            $this->process_upsert_sasaran_rpjmd_esakip($data['sasaran'], $tahun_anggaran);
            $this->process_upsert_program_rpjmd_esakip($data['program'], $tahun_anggaran);
	...
	private function deactivate_existing_rpjmd_lokal(int $tahun_anggaran)
	{
		global $wpdb;
		
		$tables_to_deactivate = [
			$this->table_data_rpjmd_visi_lokal => 'Visi RPJMD',
			$this->table_data_rpjmd_misi_lokal => 'Misi RPJMD',
			$this->table_data_rpjmd_tujuan_lokal => 'Tujuan RPJMD',
			$this->table_data_rpjmd_sasaran_lokal => 'Sasaran RPJMD',
			$this->table_data_rpjmd_program_lokal => 'Program RPJMD',
		];

		$where_clause = ['active' => 1, 'tahun_anggaran' => $tahun_anggaran];
	...
	private function process_misi_detail_esakip(array $datas)
    {
        global $wpdb;

        $return = [];
        foreach ($datas as $v) {
			$return[$v['id_tujuan']] = $v['id_misi'];
        }

        return $return;
    }

	private function process_upsert_visi_rpjmd_esakip(array $data, int $tahun_anggaran)
    {
        global $wpdb;
        $id_map = [];
        ...
        foreach ($data as $v) {
            ...
            $existing_id = $wpdb->get_var(
				$wpdb->prepare("
					...
					WHERE visi_teks = %s
					  AND tahun_anggaran = %d
				", $v['visi'], $tahun_anggaran)
			);

            if ($existing_id) {
                $results = $wpdb->update($this->table_data_rpjmd_visi_lokal, $datas, ['id' => $existing_id]);
                $id_map[$v['id']] = $existing_id; 
            } else {
                $results = $wpdb->insert($this->table_data_rpjmd_visi_lokal, $datas);
	...
	private function process_upsert_misi_rpjmd_esakip(array $data, int $tahun_anggaran, array $visi_id_map)
    {
        global $wpdb;
        $id_map = [];
        ...
        foreach ($data as $v) {
            ...
            $existing_id = $wpdb->get_var(
				$wpdb->prepare("
					...
					WHERE misi_teks = %s 
					  AND tahun_anggaran = %d
				", $v['misi'], $tahun_anggaran)
			);

            if ($existing_id) {
                $results = $wpdb->update($this->table_data_rpjmd_misi_lokal, $datas, ['id' => $existing_id]);
                $id_map[$v['id']] = $existing_id;
            } else {
                $results = $wpdb->insert($this->table_data_rpjmd_misi_lokal, $datas);
                $id_map[$v['id']] = $wpdb->insert_id;
            }
	...
	private function process_upsert_tujuan_rpjmd_esakip($data, $tahun_anggaran, $misi_id_map, $misi_detail)
	{
		global $wpdb;

        foreach ($data as $v) {
            $new_misi_id = $misi_id_map[$misi_detail[$v['id_unik']]] ?? null;
            ...
            if ($existing_id) {
                $result = $wpdb->update($this->table_data_rpjmd_tujuan_lokal, $datas, ['id' => $existing_id]);
            } else {
                $result = $wpdb->insert($this->table_data_rpjmd_tujuan_lokal, $datas);
            }
	...
	private function process_upsert_sasaran_rpjmd_esakip($data, $tahun_anggaran)
	{
		global $wpdb;
        ...
        foreach ($data as $v) {
            ...
            if ($existing_id) {
                $result = $wpdb->update($this->table_data_rpjmd_sasaran_lokal, $datas, ['id' => $existing_id]);
            } else {
                $result = $wpdb->insert($this->table_data_rpjmd_sasaran_lokal, $datas);
            }
	...
	private function process_upsert_program_rpjmd_esakip($data, $tahun_anggaran)
	{
		global $wpdb;
		...
		foreach ($data as $v) {
			...
			if ($existing_id) {
				$result = $wpdb->update($this->table_data_rpjmd_program_lokal, $datas, ['id' => $existing_id]);
			} else {
				$result = $wpdb->insert($this->table_data_rpjmd_program_lokal, $datas);
			}
	...
	private function process_upsert_tujuan_rpd(int $id_jadwal, int $tahun_anggaran)
	{
		...
		foreach ($data_lokal as $v) {
			...
			if ($existing_id) {
				$results = $wpdb->update($this->table_data_rpd_tujuan, $datas, ['id' => $existing_id]);
			} else {
				$results = $wpdb->insert($this->table_data_rpd_tujuan, $datas);
			}
	...
	private function process_upsert_sasaran_rpd(int $id_jadwal, int $tahun_anggaran)
	{
		...
		foreach ($data_lokal as $v) {
			...
			if ($existing_id) {
				$results = $wpdb->update($this->table_data_rpd_sasaran, $datas, ['id' => $existing_id]);
			} else {
				$results = $wpdb->insert($this->table_data_rpd_sasaran, $datas);
			}
	...
	private function process_upsert_program_rpd(int $id_jadwal, int $tahun_anggaran)
	{
		...
		foreach ($data_lokal as $v) {
			...
			if ($existing_id) {
				$results = $wpdb->update($this->table_data_rpd_program, $datas, ['id' => $existing_id]);
			} else {
				$results = $wpdb->insert($this->table_data_rpd_program, $datas);
			}
	...
	private function process_upsert_tujuan_rpjmd(int $id_jadwal, array $misi_id_map, int $tahun_anggaran)
    {
        ...
        foreach ($data_lokal as $v) {
            ...
            if ($existing_id) {
                $result = $wpdb->update($this->table_data_rpjmd_tujuan, $datas, ['id' => $existing_id]);
            } else {
                $result = $wpdb->insert($this->table_data_rpjmd_tujuan, $datas);
            }
	...
	private function process_upsert_sasaran_rpjmd(int $id_jadwal, int $tahun_anggaran)
    {
        ...
        foreach ($data_lokal as $v) {            
           $datas = [
            ...
            if ($existing_id) {
                $result = $wpdb->update($this->table_data_rpjmd_sasaran, $datas, ['id' => $existing_id]);
            } else {
                $result = $wpdb->insert($this->table_data_rpjmd_sasaran, $datas);
            }
	...
	private function process_upsert_program_rpjmd(int $id_jadwal, int $tahun_anggaran)
	{
		...
		foreach ($data_lokal as $v) {
			...
			if ($existing_id) {
				$result = $wpdb->update($this->table_data_rpjmd_program, $datas, ['id' => $existing_id]);
			} else {
				$result = $wpdb->insert($this->table_data_rpjmd_program, $datas);
			}
